...
프랜차이즈 매장의 전체 주소 및 GPS 좌표, 일별 운영 설정(영업시간, 픽업/딜리버리 활성화 여부, 다음 주문 가능 시간 슬롯), 시간대 및 로케일 설정, 내부 매장 상태(ACTIVE), 기능 플래그(어떤 플랫폼 기능이 각 매장에서 활성화돼 있는지), 프로모션 레코드(가장 최근 업데이트 타임스탬프가 2026년 2월로 확인됨)가 포함됐다.

**가맹점 PII**

가맹점주 성명, 실제 물리 주소, 이메일 주소가 포함됐다. 이 정보는 표적형 소셜 엔지니어링 및 피싱 공격의 재료가 된다.

**결제 통합 자격증명 (가장 심각한 항목)**

Worldpay Access 설정 및 Apple Pay, Google Pay 가맹점 ID, Stripe pk_live 퍼블리셔블 키(프로덕션 환경 키), Sentry DSN(Data Source Name)이 포함됐다. Stripe 퍼블리셔블 키는 설계상 클라이언트 사이드에 노출되는 키이나, 가맹점 ID와 Sentry DSN과 결합되면 공격 표면이 크게 확장된다. Sentry DSN 유출 시 공격자는 가짜 텔레메트리 주입, 애플리케이션 에러 모니터링, 백엔드 인프라 정보 수집이 가능해진다.
...
데이터셋에 Wendy's 외에 Sbarro, KFC 등 여러 브랜드의 데이터가 혼재하고, 데이터베이스 스키마가 알려진 온라인 주문 백엔드 구조와 정확히 일치하며, Cybernews 연구팀이 QikServe by The Access Group을 공통 기반 인프라로 특정했다. QikServe는 8,000개 이상 매장, 연간 £30억 이상의 거래를 처리하는 SaaS 플랫폼이다. Wendy's와 The Access Group 양측 모두 공식 성명을 발표하지 않았으므로, QikServe 플랫폼 침해 여부는 현재 공식 확인되지 않은 상태다.
...
Stripe 퍼블리셔블 키 단독으로는 결제 처리가 불가능하지만, 가맹점 ID와 결합되면 허위 결제 흐름 생성이 가능하다. Sentry DSN 접근은 애플리케이션 에러 로그 열람을 통한 백엔드 인프라 정보 수집 및 가짜 에러 이벤트 주입으로 탐지 회피에 악용될 수 있다. Worldpay Access 설정 노출은 결제 게이트웨이 설정 변경 시도의 위험이 있다.
...
민감 자격증명과 운영 데이터, PII의 분리 저장 구조로 재설계가 필요하다. SaaS 공급업체 선정 및 계약 시 보안 평가 체크리스트와 자격증명 관리 정책 적용을 계약 조건으로 명시해야 한다. 프랜차이즈 모델에서 중앙 플랫폼 침해 시 피해 범위를 제한하기 위한 가맹점별 자격증명 분리도 검토가 필요하다.
