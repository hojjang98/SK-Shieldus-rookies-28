# HackerOne / Navia BOLA 취약점 침해 — API 보안 결함이 버그바운티 플랫폼 직원 정보를 노출하다
...
### 2-1. BOLA(Broken Object Level Authorization) 취약점의 정의 및 위험성
...
피해 직원 및 피부양자의 데이터: 사회보장번호(SSN), 전체 이름, 주소, 전화번호, 생년월일, 이메일 주소, 의료 플랜 참여 여부, 비의료 플랜 참여 여부, 플랜 가입일, 유효 시작일, 종료일.
...
복리후생 관리 시스템과 같이 대규모 개인정보를 처리하는 서드파티 SaaS 서비스에 대한 보안 요구 사항 검토가 계약 단계에서 충분히 이루어지지 않은 경향이 있다. 이는 비기술 부서(HR, 총무)가 보안팀의 검토 없이 서비스를 계약하는 경우에 자주 발생한다.
...
**즉각적 조치**
피해 직원들에게 신용 모니터링 서비스를 즉시 제공해야 한다. 3개 주요 신용 기관(Equifax, Experian, TransUnion)에 신용 동결(credit freeze) 또는 사기 경보(fraud alert) 설정을 권고해야 한다. SSN 노출 직원들은 IRS Identity Protection PIN 신청을 고려해야 한다.

**단기 조치**
HR 및 복리후생 서비스에 활용되는 서드파티 벤더들의 API 보안 수준을 평가해야 한다. 계약 단계에서 SOC 2 Type II 보고서, 정기 침투 테스트 결과, API 보안 평가 결과 제출을 요구 사항에 포함해야 한다.

**장기 조치**
자체 서비스의 모든 API 엔드포인트에 대한 BOLA 취약점 점검을 수행해야 한다. 자동화 스캐너만으로는 BOLA를 완전히 탐지하기 어려우므로, API 로직에 대한 수동 보안 검토를 보안 평가 방법론에 포함해야 한다. API 게이트웨이에서 특정 객체 식별자에 대한 비정상적인 접근 빈도를 탐지하는 이상 탐지 규칙을 설정해야 한다.
...
A. 방어는 서버 측 코드 레벨에서 이루어져야 한다. 모든 API 요청에서 요청된 객체의 소유자가 현재 인증된 사용자와 일치하는지를 데이터베이스 수준에서 검증하는 쿼리 패턴을 사용해야 한다. 예를 들어, SELECT * FROM records WHERE id = {requested_id} 대신 SELECT * FROM records WHERE id = {requested_id} AND owner_id = {authenticated_user_id} 형태로 권한 검증을 쿼리에 내장해야 한다. 또한 API 게이트웨이에서 단일 사용자가 짧은 시간 내에 다수의 서로 다른 객체 ID를 조회하는 패턴을 이상 탐지 규칙으로 설정해야 한다.
...
이번 사건에서 가장 중요한 학습 포인트는 BOLA 취약점의 특성이다. BOLA는 공격자가 특별한 기술 없이도, 합법적으로 인증된 계정 하나만으로 시스템 내 모든 다른 사용자 데이터에 접근할 수 있게 한다. 자동화된 공격이 아닌 수동 파라미터 변조로도 충분히 악용 가능하므로, 정교한 공격 도구가 필요하지 않다. 그럼에도 불구하고 탐지가 어렵고 보안 평가에서 자주 누락된다는 점에서 실제 위험성이 매우 높다.

또한 이번 사건은 보안 사고 발생 시 피해자 통보의 적시성이 법적, 신뢰적 측면에서 모두 중요함을 보여준다. 침해를 탐지한 후 피해 기업에 통보하는 데 약 4주가 소요되었고, 이 서한이 실제로 수령되기까지 추가 시간이 걸렸다. 서드파티 계약에 명확한 통보 기한과 전달 확인 메커니즘을 명시하는 것이 필수적이다.
