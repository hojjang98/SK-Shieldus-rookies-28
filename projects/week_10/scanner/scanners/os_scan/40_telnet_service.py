...
def scan(target_url, login_info=None):
    ...
    try:
        ...
        if os_type == "Linux":
            ...
            inetd_on = _inetd_enabled(inetd_conf, "telnet")

            xinetd_conf = execute_command(target_url, session, "cat /etc/xinetd.d/telnet 2>/dev/null")
            xinetd_on = _xinetd_enabled(xinetd_conf) if xinetd_conf.strip() else False

            socket_output = execute_command(
                target_url,
                session,
                "systemctl list-units --type=socket --state=active | grep telnet 2>&1"
            )
            socket_on = bool(socket_output.strip())

            if inetd_on or xinetd_on or socket_on:
                result['vulnerable'] = True
                result['details'].append("[OS] Telnet 서비스가 활성화되어 있음")
                result['recommendations'].append("Telnet 서비스 비활성화")
            else:
                result['details'].append("[OS] Telnet 서비스 비활성화: 양호")

        if not result['recommendations']:
            result['recommendation'] = '안전 - Telnet 서비스가 비활성화됨'
        else:
