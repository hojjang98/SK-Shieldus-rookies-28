    ...
    try:
        ...
        if os_type == "Linux":
            ...
            systemd_output = execute_command(
                target_url,
                session,
                "systemctl list-units --type=service --state=active | grep -E 'echo|discard|daytime|chargen' 2>&1"
            )
            systemd_on = bool(systemd_output.strip())

            if inetd_on or xinetd_on or systemd_on:
                result['vulnerable'] = True
                result['details'].append("[OS] DoS 취약 서비스가 활성화되어 있음")
                result['recommendations'].append("echo/discard/daytime/chargen 서비스 비활성화")
            else:
                result['details'].append("[OS] DoS 취약 서비스 비활성화: 양호")

        if not result['recommendations']:
            result['recommendation'] = '안전 - DoS 취약 서비스가 비활성화됨'
        else:
