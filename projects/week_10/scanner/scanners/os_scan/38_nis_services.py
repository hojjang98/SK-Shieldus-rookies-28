    ...
    try:
        ...
        if os_type == "Linux":
            output = execute_command(
                target_url,
                session,
                "systemctl list-units --type=service --state=active | grep -E 'ypserv|ypbind|ypxfrd|rpc.yppasswdd|rpc.ypupdated' 2>&1"
            )
            if output.strip():
                result['vulnerable'] = True
                result['details'].append("[OS] NIS/NIS+ 서비스 활성화됨")
                result['recommendations'].append("NIS/NIS+ 서비스 비활성화")
            else:
                result['details'].append("[OS] NIS/NIS+ 서비스 비활성화: 양호")

        if not result['recommendations']:
            result['recommendation'] = '안전 - NIS/NIS+ 서비스가 비활성화됨'
        else:
