    ...
    try:
        ...
        if os_type == "Linux":
            ...
            if inetd_on or xinetd_on:
                ...
                result['details'].append("[OS] r 계열 서비스가 활성화되어 있음")
                result['recommendations'].append("r 계열 서비스 비활성화 필요")
            else:
                result['details'].append("[OS] r 계열 서비스 비활성화: 양호")

        if not result['recommendations']:
            result['recommendation'] = '안전 - r 계열 서비스가 비활성화됨'
        else:
