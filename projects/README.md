...
| 주차 | 프로젝트명 | 주요 내용 |
|------|-------------|-----------|
| **Week 1** | Mini Security Log Monitor | 로그 파일(`system.log`)의 오류 이벤트를 탐지하고, 접근 결과를 JSON(`access.log`) 형식으로 기록하는 보안 감사 시스템 |
...
| **Week 7** | AWS 3-Tier 웹 인프라 완전 구축 | 프로덕션 수준의 완전한 3-Tier 아키텍처 구축. Multi-AZ VPC 네트워크 설계 (6개 Subnets, NAT Gateway 2개), Application Load Balancer 및 Auto Scaling Group 구성, RDS MySQL 데이터베이스 구축, Security Groups 계층별 보안 설정. 실제 구축 시간 3시간, 비용 ~$0.5로 고가용성 웹 인프라 완성 |
| **Week 8** | 웹 해킹 기초 - 취약한 웹 애플리케이션 구축 및 공격 실습 | **OWASP Top 10** 기반 의도적으로 취약한 Flask 웹 애플리케이션 개발. SQL Injection 로그인 우회 (`admin' --`), XSS 공격 (`<script>alert('XSS')</script>`) 직접 시연. Prepared Statement 및 HTML Escape 방어 기법 학습. 로그인 + 게시판 기능 구현, SQLite DB 사용. 보안의 중요성 체감 및 안전한 코딩 실전 경험 |
...
| **Week 17** | 수탁사 현황 시나리오 스케치 | 파이널 프로젝트 수탁사 후보 9개 시나리오 초안 작성. 위탁사: 하랑항공 (연간 탑승객 1,500만명, 회원 800만명). 수탁사별 위탁 업무·처리 개인정보·주요 컴플라이언스 이슈 정의. 케이터링(하랑케이터링) · 클라우드(스카이클라우드) · 콜센터 · 물류 · 금융 등 업종별 현실적 취약점 도출. 개인정보보호법 제26조(재위탁 사전 승인), 제23조(민감정보), 제21조(파기) 중심 법적 이슈 매핑 |
| **Week 18** | 파이널 프로젝트 팀 협업 준비 (libero-project) | 팀 킥오프 및 역할 분담. 수탁사 시나리오 최종 확정 목표. 팀장 사전 준비 자료 제작: WBS 예시(하랑케이터링 기준), 점검 체크리스트 53개 항목, 점검보고서 포맷, 증적자료 예시 5종(HTML). 팀원 담당 수탁사 배정, WBS·수행계획서 취합 계획 수립 |
| **Week 19** | 산출물 목록 확정 및 진행 현황 정리 | 5주 프로젝트 전체 산출물 점검. 완료/진행중/미착수/제외 항목 분류 (WBS·수행계획서 수정 중, 위수탁 계약서·수탁사 목록·점검계획서 미착수, 요구사항 정의서·RFP 제외). 수탁사 시나리오 진행중, 체크리스트·결과보고서 수정 중, 개선방안·교육계획서 미착수. 리베로 사업제안서 수정 중 확인 및 다음 주 우선순위 확정 |
| **Week 20** | [파이널] 착수 및 계획 수립 | WBS(5주 일정) 및 수행계획서 완성. 리베로 사업제안서 ver2 작성. 수탁사 6개사 목록 확정(다온컨택·온다로지스·패스트레인·플라이메이트·하랑케이터링·해랑금융). 데일리 스크럼 운영 시작. 멘토 제공 참고자료 및 26기·23기 사례 검토 |
| **Week 21** | [파이널] 수탁사 분석 및 점검 계획 수립 | 6개 수탁사 현황 문서 작성 (업종별 개인정보 처리 현황). 위험도 기반 수탁사 등급 산정 및 개인정보 범주 통합 매핑. 수탁사별 점검계획서 6종 작성 (점검 일정·범위·방법). 하랑항공-수탁사 간 개인정보처리위탁계약서 6종 작성 (개인정보보호법 제26조 2항 필수 기재사항 반영). 사업제안서 ver3 피드백 반영 개정 |
| **Week 22** | [파이널] 현장 점검 실시 및 증적 수집 | 체크리스트 기반 6개 수탁사 현장 점검 수행. 수탁사별 증적 자료 수집: 다온컨택(암호화·패치), 온다로지스(파기·로그), 패스트레인(DLP·교육), 플라이메이트(마스킹·백신), 하랑케이터링(물리보안·파기대장), 해랑금융(DB암호화·접근통제·재위탁). 결함/관찰사항/양호 판정 분류. 업종별 특성에 따른 취약점 패턴 도출 |
| **Week 23** | [파이널] 결과 분석 및 보고서 작성 | 수탁사별 점검결과보고서 6종 작성 (결함 유형·위험도 평가). 수탁사별 개선방안보고서 6종 작성 (실현 가능성 중심 권고사항). 수탁사 교육 계획서 및 교육 자료 제작. 리베로(위탁사) 전체 점검결과보고서 통합본 작성. 사업제안서 v4 최종화 |
| **Week 24** | [파이널] 최종 발표 및 프로젝트 마무리 | 5주간 수탁사 점검 프로젝트 최종 발표 준비 및 진행. 발표 자료 PPT v3.5 → v4.3 완성. 전체 프로젝트 회고: 6개사 점검, 개인정보보호법 제26조 기반 위·수탁 관리체계 구축 완료. 산출물: WBS·수행계획서·사업제안서·수탁사현황·점검계획서·위탁계약서·체크리스트·증적자료·결과보고서·개선방안보고서·교육자료 일체 |
