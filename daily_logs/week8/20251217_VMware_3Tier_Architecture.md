### 📄 2024.12.17 (Day 35) [VMware 3-Tier Architecture 구성 실습]
...
VMware로 3-Tier Architecture를 직접 구축하면서, **"보안은 계층(Layer)로 구현된다"**는 핵심 원칙을 체감했습니다. 각 계층은 독립적으로 작동하지만, 서로 정의된 인터페이스(포트/프로토콜)로만 통신합니다. 한 계층이 침투당해도 다른 계층은 방화벽 규칙으로 보호됩니다.

특히 **리버스 프록시의 전략적 가치**를 깨달았습니다. Nginx가 단순히 HTTP 요청을 전달하는 게 아니라, SSL 종료, Rate Limiting, IP 필터링, 로그 수집을 한 곳에서 처리함으로써 백엔드 서버를 보호하고 관리를 단순화합니다. 이것이 실무에서 로드 밸런서나 API Gateway를 사용하는 이유입니다.

**수업에서 배운 명령어를 직접 구현하면서**, 단순히 명령어를 실행하는 것과 그 의미를 이해하며 실행하는 것의 차이를 느꼈습니다. 예를 들어 `bind-address = 0.0.0.0`으로 변경하는 순간, 데이터베이스가 모든 외부 IP에서 접근 가능해진다는 보안 위험을 실시간으로 체감했습니다. 이런 경험이 실무에서 설정 실수를 방지하는 감각을 길러줄 것 같습니다.

가장 큰 수확은 **"이론 → 실습 → 이해 → 확장"의 학습 사이클**을 경험한 것입니다. 3-Tier 아키텍처 개념을 이론으로 배우고, VMware로 직접 구현하고, 각 계층의 역할을 이해하고, 이제는 Docker나 Kubernetes로 확장할 수 있는 기초를 다졌습니다.
...
**[검증 1] 네트워크 연결성**
```bash
# 시나리오: 3대의 VM이 같은 네트워크에 있는가?
# VMware NAT 또는 Bridge 모드에서:
# - VM1: 192.168.186.128 (Web Server)
# - VM2: 192.168.186.129 (WAS Server)  
# - VM3: 192.168.186.130 (DB Server)

# 검증 방법:
VM1에서: ping 192.168.186.129 && ping 192.168.186.130
VM2에서: ping 192.168.186.128 && ping 192.168.186.130
VM3에서: ping 192.168.186.128 && ping 192.168.186.129

# 논리적 검토: ✓ 올바름
# VMware NAT는 192.168.X.0/24 대역을 기본 할당
# 같은 네트워크에 있으므로 라우팅 없이 직접 통신 가능
```

**[검증 2] Nginx → Tomcat 프록시**
```nginx
...
server {
    ...
    location / {
        index index.jsp index.html index.htm;
        proxy_pass http://192.168.186.129:8080;  # ← VM2의 Tomcat
...
# 논리적 검토: ✓ 올바름
# 1. 클라이언트가 http://192.168.186.128/ 요청
# 2. Nginx가 80 포트로 수신
# 3. proxy_pass로 http://192.168.186.129:8080/로 전달
# 4. Tomcat이 8080에서 응답 생성
# 5. Nginx가 클라이언트에게 응답 반환

# 주의: Tomcat은 0.0.0.0:8080에서 리스닝해야 함 (기본값)
```

**[검증 3] Tomcat → MariaDB JDBC 연결**
```jsp
<%
    String dbURL = "jdbc:mysql://192.168.186.130:3306/testdb";
    String dbUser = "mydb";
...
# 논리적 검토: ✓ 올바름 (단, 조건 충족 필요)
# 전제조건:
# 1. MariaDB bind-address = 0.0.0.0 (또는 192.168.186.0)
# 2. 사용자 'mydb'@'%' 생성 및 권한 부여
# 3. 방화벽 3306 포트 허용 (또는 비활성화)
# 4. JDBC 드라이버 /opt/tomcat/lib/에 존재
# 5. testdb 데이터베이스 존재

# 연결 흐름:
# Tomcat(192.168.186.129) → TCP 3306 → MariaDB(192.168.186.130)
```

**[검증 4] 방화벽 규칙 논리**
```bash
# VM1 (Web Server) - 외부에서 접근
ufw allow 80/tcp        # HTTP: 모든 클라이언트 허용
ufw allow 22/tcp        # SSH: 관리자 접근

# VM2 (WAS Server) - Web에서만 접근
ufw allow from 192.168.186.128 to any port 8080  # Nginx만 접근
ufw allow 22/tcp        # SSH: 관리자 접근

# VM3 (DB Server) - WAS에서만 접근
ufw allow from 192.168.186.129 to any port 3306  # Tomcat만 접근
ufw allow 22/tcp        # SSH: 관리자 접근

# 논리적 검토: ✓ 올바름
# 계층별 접근 제어가 명확히 구현됨
# 외부 → Web만 가능, Web → WAS만 가능, WAS → DB만 가능
```

**[검증 5] 포트 충돌 가능성**
```bash
# 각 VM에서 실행되는 서비스:
# VM1: Nginx (80), sshd (22)
# VM2: Tomcat (8080), sshd (22)
# VM3: MariaDB (3306), sshd (22)

# 논리적 검토: ✓ 충돌 없음
# 각 VM은 독립된 네트워크 스택을 가지므로
# 같은 포트 번호(22)를 각 VM에서 사용 가능
# 다른 VM끼리는 IP로 구분됨
```

### 🔍 잠재적 문제 및 해결 방법

**[문제 1] MariaDB 원격 접속 실패**
```bash
# 증상: Tomcat에서 DB 연결 시 "Connection refused"

# 원인 분석:
1. bind-address가 127.0.0.1로 설정됨 (localhost만 허용)
2. 방화벽이 3306 포트 차단
3. 사용자 권한이 'mydb'@'localhost'로 제한됨
...
mysql -u root -p
> SHOW GRANTS FOR 'mydb'@'%';
# 'mydb'@'192.168.186.129' 또는 'mydb'@'%' 있어야 함

# 검증:
# VM2에서: mysql -u mydb -p -h 192.168.186.130
# 연결 성공하면 OK
```

**[문제 2] Nginx 프록시 502 Bad Gateway**
```bash
# 증상: 브라우저에서 502 Bad Gateway 오류

# 원인 분석:
1. Tomcat이 실행되지 않음
2. Tomcat이 127.0.0.1:8080만 리스닝 (0.0.0.0:8080 아님)
3. 방화벽이 8080 포트 차단
4. Nginx 설정에서 잘못된 IP/포트 지정

# 해결 방법:
# 1. Tomcat 상태 확인
systemctl status tomcat
netstat -antp | grep 8080
# 0.0.0.0:8080 또는 :::8080으로 리스닝하는지 확인
...
# 3. 방화벽 확인 (VM2에서)
ufw status
# 8080이 허용되어 있는지, 또는 비활성화되어 있는지 확인

# 4. Nginx 설정 확인 (VM1에서)
nginx -t  # 문법 검사
cat /etc/nginx/sites-available/default | grep proxy_pass
# http://192.168.186.129:8080 맞는지 확인

# 5. Nginx 에러 로그 확인
tail -f /var/log/nginx/error.log
# "connect() failed" 메시지로 원인 파악
```

**[문제 3] JSP 페이지 404 Not Found**
```bash
# 증상: http://192.168.186.128/dbtest.jsp → 404

# 원인 분석:
1. JSP 파일이 잘못된 경로에 위치
2. Tomcat webapps 디렉토리 구조 오류
3. 파일 권한 문제 (tomcat 사용자가 읽기 불가)

# 해결 방법:
# 1. 파일 위치 확인 (VM2에서)
ls -la /opt/tomcat/webapps/ROOT/
# index.jsp, dbtest.jsp 존재 확인

# 2. 파일 권한 확인
ls -l /opt/tomcat/webapps/ROOT/dbtest.jsp
# -rw-r--r-- tomcat tomcat 형태여야 함

# 3. 권한 수정 (필요 시)
chown tomcat:tomcat /opt/tomcat/webapps/ROOT/dbtest.jsp
chmod 644 /opt/tomcat/webapps/ROOT/dbtest.jsp
...
**[문제 4] JDBC 드라이버 ClassNotFoundException**
```bash
# 증상: JSP 실행 시 "java.lang.ClassNotFoundException: com.mysql.cj.jdbc.Driver"

# 원인: JDBC 드라이버 JAR 파일이 없거나 잘못된 위치
...
# 3. 소유권 변경
chown tomcat:tomcat /opt/tomcat/lib/mysql-connector-j-8.3.0.jar

# 4. Tomcat 재시작
systemctl restart tomcat

# 주의: 
# - 드라이버는 /opt/tomcat/lib/ (공통) 또는
#   /opt/tomcat/webapps/ROOT/WEB-INF/lib/ (앱 전용)에 위치
# - 최신 MySQL Connector/J는 'com.mysql.cj.jdbc.Driver' 사용
#   (구버전은 'com.mysql.jdbc.Driver')
```
...
# VM1 (Web Server)
curl http://localhost
# 결과: Nginx 기본 페이지 또는 프록시된 Tomcat 페이지

# VM2 (WAS Server)
curl http://localhost:8080
# 결과: Tomcat JSP 페이지 (현재 서버 시간 등 표시)

# VM3 (DB Server)
mysql -u mydb -p -h localhost
# 결과: MariaDB 프롬프트 접속 성공
...
# VM1에서:
curl http://192.168.186.129:8080
# 결과: Tomcat JSP 페이지 (직접 접근)

# VM2 → VM3
# VM2에서:
mysql -u mydb -p -h 192.168.186.130
# 결과: DB 연결 성공, testdb 데이터 조회 가능

# [Step 3] 전체 흐름 통합 테스트

# 호스트 PC에서 Web Server로 접속:
curl http://192.168.186.128/

# 예상 결과: Tomcat JSP 페이지 (Nginx 프록시 경유)
...
# 예상 결과: 
# - "데이터베이스 연결 성공!" 메시지
# - users 테이블 데이터 HTML 테이블로 표시

# [Step 4] 부하 테스트 (선택)
# Apache Bench로 동시 접속 테스트
ab -n 1000 -c 10 http://192.168.186.128/

# 예상 결과:
# - Requests per second 확인
# - Failed requests: 0 (모든 요청 성공)
```

### 논리적 일관성 최종 점검

1. **네트워크 계층** ✓
   - 3대 VM이 동일 서브넷에 있음 (192.168.186.0/24)
   - 라우터 없이 직접 통신 가능
   - 각 VM은 고유 IP 보유

2. **애플리케이션 계층** ✓
   - Nginx는 HTTP 프록시 역할 수행
   - Tomcat은 JSP/Servlet 실행
   - MariaDB는 데이터 저장/조회

3. **포트 바인딩** ✓
   - Nginx: 0.0.0.0:80 (모든 인터페이스)
   - Tomcat: 0.0.0.0:8080 (기본 설정)
   - MariaDB: 0.0.0.0:3306 (설정 변경 후)

4. **인증/인가** ✓
   - Nginx: 인증 없음 (퍼블릭 접근)
   - Tomcat: 세션 기반 (구현 시)
   - MariaDB: 사용자/비밀번호 ('mydb' / 'abcd1234')
...
**결론**: 제시된 아키텍처와 설정은 **논리적으로 일관성 있고 실제 구현 가능**합니다. 수업에서 제공된 명령어를 바탕으로 직접 구현하면 정상 동작합니다.
