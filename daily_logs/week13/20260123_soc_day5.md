# 📄 2025.01.23 (Day 17) - Splunk Forwarder 구성 및 Wazuh HIDS 환경 구축
...
### 실습 17-1: Ubuntu Forwarder 설치 및 로그 전송 설정
...
**실습 단계:**
```bash
# 1. Splunk Universal Forwarder 설치
dpkg -i splunkforwarder-8.2.0-e053ef3c985f-linux-2.6-amd64.deb

# 2. SWID 태그 복사 (소프트웨어 식별 태그)
cp /opt/splunkforwarder/swidtag/splunk-UniversalForwarder-primary.swidtag \
   /opt/splunkforwarder/etc/regid.2001-12.com.splunk-UniversalForwarder.swidtag

# 3. 부팅 시 자동 시작 설정
/opt/splunkforwarder/bin/splunk enable boot-start

# 4. Splunk 서버로 데이터 전송 설정
/opt/splunkforwarder/bin/splunk add forward-server 192.168.2.x:9997

# 5. 어떤 로그를 수집할지 정의 (inputs.conf 수정)
...
### 실습 17-2: Splunk 서버 리스닝 포트 활성화

**목표:** Splunk 서버에서 Forwarder 데이터 수신 준비

**실습 환경:**
- Splunk Server: 192.168.2.x
- 리스닝 포트: 9997 (Splunk 기본 수신 포트)

**실습 단계:**
```bash
# 1. Splunk 서버에서 9997 포트 리스닝 활성화
...
### 실습 17-3: 로그 수집 검증 및 공격 시뮬레이션

**목표:** Forwarder가 정상적으로 로그를 수집하는지 확인하고 Kali Linux에서 공격 시뮬레이션

**실습 환경:**
- Ubuntu Forwarder: 로그 수집 대상
- Kali Linux: 공격 시뮬레이션 수행
- Splunk Server: 실시간 로그 분석

**실습 단계:**
```bash
# 1. 로그 파일 라인 수 확인 (수집 전)
wc -l /var/log/ufw.log
wc -l /var/log/nginx/access.log
wc -l /var/log/auth.log

# 2. 방화벽 활성화 및 SSH 포트 허용
ufw enable
ufw allow 22/tcp
reboot
...
# Kali에서 실행:
for i in {1..15}; do
    ssh fakeuser@192.168.2.y
done

# 5. 로그 파일 라인 수 재확인 (수집 후)
wc -l /var/log/auth.log
# → "Failed password" 로그가 증가했는지 확인
```
...
### 실습 17-4: Windows Forwarder 설치 및 프록시 설정
...
**실습 단계:**
```powershell
# 1. Splunk Universal Forwarder 설치 (Windows용)
# 설치 파일 실행 후 관리자 권한으로 설치
...
# 5. Splunk Web UI 접속 테스트
# http://192.168.2.x:8000
```
...
```spl
# Windows 호스트에서 수집된 데이터 확인
index=* host="DESKTOP-IRB3M7S"
| stats count by source sourcetype

# Security 이벤트 로그 확인
index=* host="DESKTOP-IRB3M7S" source="WinEventLog:Security"
| head 20
...
hostnamectl set-hostname wazuh-server   # .10
hostnamectl set-hostname wazuh-agent1   # .11
hostnamectl set-hostname wazuh-agent2   # .12
...
# 인덱스 분리 전략
[monitor:///var/log/auth.log]
index = waf          # 인증/접근 제어 관련
sourcetype = access_combined

[monitor:///var/log/nginx/access.log]
index = web          # 웹 애플리케이션 로그
sourcetype = access_combined

[monitor:///var/log/ufw.log]
index = main         # 시스템/방화벽 로그
sourcetype = access_combined
...
|:---:|:---|:---|:---|
| **초기 침투** | - 다수 인증 실패<br>- 알려지지 않은 IP<br>- 비정상 접속 시간 | `Failed password for admin from 203.x.x.x` | - 해당 IP 즉시 차단<br>- 계정 잠금 정책 적용<br>- 위협 인텔리전스 조회 |
| **정찰 활동** | - 404 에러 다발<br>- 디렉터리 스캔<br>- SQL Injection 시도 | `GET /../../../etc/passwd 404` | - WAF 정책 업데이트<br>- 공격 IP 블랙리스트 등록<br>- 취약점 긴급 패치 |
| **권한 상승** | - /etc/passwd 변경<br>- sudo 사용 급증<br>- 계정 추가 | `useradd hacker` | - 변경사항 즉시 롤백<br>- 침해 계정 격리<br>- 포렌식 이미지 확보 |
...
("union select" OR "select * from" OR "etc/passwd" OR "<script")
| stats count by clientip, uri
| sort - count

# 3. 404 에러 다발 (디렉터리 스캔 의심)
...
<rule id="100002" level="12">
  ...
  <description>User account file modified</description>
  <group>syscheck,pci_dss_10.2.7</group>
</rule>
```
...
- **프록시 설정의 함정:** 기업 환경에서 흔히 설정된 프록시가 내부망 서비스 접속까지 방해할 수 있다는 점을 처음 알았다. "로컬 주소에 프록시 사용 안 함" 설정의 중요성을 실감했고, 이는 보안 솔루션 구축 시 반드시 확인해야 할 체크리스트 항목이다.

- **인덱스 분리 전략:** 로그 유형별로 인덱스를 분리(waf, web, main)하면 검색 성능뿐만 아니라 접근 제어, 데이터 보존 정책까지 세밀하게 관리할 수 있다는 점을 배웠다. 실무에서는 조직의 규제 준수 요구사항에 맞춰 인덱스 전략을 수립해야 한다.

- **Wazuh Agentless의 실용성:** 에이전트 설치가 불가능한 레거시 시스템이나 임베디드 장비에서도 SSH를 통한 원격 명령 실행으로 보안 모니터링을 구현할 수 있다는 점이 매우 실용적이었다. 특히 /etc/passwd, /etc/shadow 같은 중요 파일의 변경을 10초마다 자동 점검하는 방식이 효과적이다.

- **호스트명 고유성의 중요성:** Wazuh에서 각 에이전트를 식별하는 기준이 호스트명이기 때문에, 중복 시 에이전트 충돌이 발생한다. 시스템 구축 초기에 네이밍 컨벤션을 확립하는 것이 얼마나 중요한지 깨달았다.

### 이전 학습과의 연결고리

- **Day 16 Splunk 기초와 연계:** 이전에 배운 SPL 쿼리 문법(stats, sort, rex)을 실제 보안 로그 분석에 적용했다. 특히 정규표현식으로 IP 주소를 추출하는 `rex` 명령어가 실무에서 얼마나 유용한지 체감했다.

- **웹 해킹 실습 확장:** 이전에 학습한 SQL Injection, XSS 공격 기법을 직접 시뮬레이션하고, nginx access.log에서 해당 패턴을 Splunk로 탐지하는 전 과정을 경험했다. 공격자와 방어자의 관점을 모두 이해할 수 있었다.
...
**보안 전문가 관점:**
- **계층적 모니터링 체계:** Splunk로 모든 로그를 수집/검색하고, Wazuh로 크리티컬 호스트(DB 서버, 도메인 컨트롤러)의 실시간 무결성을 점검하는 이중 방어 체계를 구축할 수 있다. DMZ의 웹 서버는 Splunk로 트래픽 분석, 내부 서버는 Wazuh로 파일 무결성 점검하는 식의 역할 분담이 효과적이다.
...
| 문제 | 원인 | 해결 방법 |
|:---|:---|:---|
| **Splunk Web UI 접속 불가** | 프록시 설정으로 로컬 주소도 프록시 경유 | - inetcpl.cpl → "로컬 주소에 프록시 사용 안 함" 체크<br>- 브라우저 캐시 삭제 후 재접속<br>- 직접 IP:포트로 접속 시도 |
| **Forwarder 데이터 미수신** | 9997 포트 방화벽 차단 또는 리스닝 미활성화 | - `ufw allow 9997/tcp` 또는 `firewall-cmd --add-port=9997/tcp`<br>- Splunk 서버에서 `splunk enable listen 9997` 실행<br>- `netstat -tuln \| grep 9997`로 리스닝 확인 |
| **Wazuh Agent 연결 실패** | 호스트명 중복 또는 서버 IP 오타 | - `hostnamectl`로 각 시스템 호스트명 고유성 확인<br>- `/var/ossec/etc/ossec.conf`에서 서버 IP 검증<br>- `systemctl restart wazuh-agent` 후 재연결 |
| **Agentless SSH 접속 실패** | SSH 키 인증 미설정 또는 비밀번호 오류 | - `ssh-keygen` 후 `ssh-copy-id wazuh@대상IP`<br>- `register_host.sh`에서 올바른 비밀번호 입력<br>- 대상 시스템에서 SSH 서비스 실행 확인 |
| **Windows 이벤트 로그 미수집** | SplunkForwarder 서비스 권한 부족 | - SplunkForwarder 서비스가 SYSTEM 계정으로 실행 중인지 확인<br>- `current_only = 0` 설정으로 과거 로그도 수집<br>- 이벤트 뷰어에서 해당 로그 접근 가능 여부 확인 |
