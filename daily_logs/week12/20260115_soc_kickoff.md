# 📄 2025.01.15 (Day 55) - 보안관제 이해 및 실무
...
- **투입 전 준비사항의 구체성:** 단순히 보안지식만 아는 게 아니라 네트워크 구성도, 보안시스템 목록, 관제 범위, 연관 부서까지 사전에 파악해야 한다는 걸 알았다. 7가지 반드시 해야 할 사항과 7가지 하지 말아야 할 사항이 실무 경험에서 나온 것 같아 인상적이었다.

- **보안관제의 법적 체계:** 국가사이버안전관리규정부터 시작해서 정보통신망법, 정보통신기반보호법, 전자금융거래법까지 다양한 법적 근거가 있다는 걸 처음 알았다. KISA-ISMS, ISO 27001, NIST 같은 국제 표준도 준수해야 한다.

- **사이버위기경보 대응의 구체성:** 단계별로 해야 할 행동이 명확하게 정의되어 있다. 관심 단계에서는 ACL 확인, 주의 단계에서는 침해사고대응팀 소집, 경계 단계에서는 네트워크 단절 검토까지. 실제 상황에서 바로 적용 가능한 수준이다.

- **정책 체계 관리의 중요성:** 탐지정책을 무작정 만드는 게 아니라 Category 분류, 긴급도 산정, 공격유형 분류 등 체계적으로 관리한다. 21개 공격 유형으로 분류하고 4단계 긴급도로 구분하는 게 인상적이었다.

- **관제 업무의 전문성:** 주간과 야간 업무가 구분되고, 각각 역할과 산출물이 명확하다. 2인1조 4조2교대 체계도 처음 들어봤는데, 24시간 365일 무중단 관제를 위한 필수 조건이다.

### 이전 학습과의 연결고리

- **웹 보안 → 공격유형 분류:** SQL Injection, XSS 같은 공격 기법들이 실제 보안관제에서는 21개 공격 유형 중 하나로 분류되어 관리된다는 걸 알았다. 이론적 공격 기법이 실무에서 어떻게 체계화되는지 확인했다.

- **네트워크 분석 → 모니터링:** Wireshark로 배운 패킷 분석이 보안관제 모니터링의 기초가 된다. 탐지 이벤트, 비정상 트래픽을 분석할 때 프로토콜 이해가 필수다.
...
**SOC 분석가 관점:**
- **체크리스트 작성:** 투입 전 7가지 확인사항을 체크리스트로 만들어서 매번 확인하면 좋겠다. 네트워크 구성도, 보안시스템 목록, 관제 범위, 보고서 종류 등을 빠뜨리지 않도록.

- **단계별 대응 매뉴얼:** 사이버위기경보 5단계별로 해야 할 행동을 요약한 참고자료를 만들어두면 실제 상황에서 빠르게 대응 가능하겠다.

- **정책 관리 도구:** 탐지정책을 21개 공격 유형으로 분류하고 4단계 긴급도로 구분해서 관리하는 표를 엑셀로 만들면 정책 최적화에 도움이 될 것 같다.

**보안관제 효율화:**
- **하지 말아야 할 것 숙지:** 초심 잃기, 자기계발 게을리, 이력서 지저분하게 만들기 등. 이 7가지를 항상 염두에 두고 근무해야겠다.

- **관제 인력 역할 명확화:** 주간과 야간 업무가 다르고, 각각 산출물도 다르다. 주간은 일일보고서, 야간은 근무일지. 이 차이를 정확히 알고 업무를 수행해야 한다.
...
| 문제 | 원인 | 해결 방법 |
|:---|:---|:---|
| 투입 후 업무 파악 어려움 | 사전 준비 부족 | - 네트워크 구성도 숙지<br>- 보안시스템 목록 확인<br>- 관제 범위 명확히 |
| 사이버위기 대응 혼란 | 단계별 절차 미숙지 | - 5단계 대응 매뉴얼 숙지<br>- 단계별 행동 요약표 작성 |
| 정책 관리 비효율 | 체계적 분류 부재 | - 21개 공격유형 분류<br>- 4단계 긴급도 구분 |
| 관제 품질 저하 | 역할 분담 불명확 | - 주간/야간 업무 구분<br>- 산출물 명확화 |

---

**Today's Insight:**

보안관제는 투입 전 준비부터 철저해야 하는 전문 분야다. 네트워크 구성도, 보안시스템 목록, 관제 범위 같은 7가지 필수 확인사항을 숙지하지 않으면 현장에서 제대로 대응할 수 없다. 법적 근거도 국가사이버안전관리규정부터 전자금융거래법까지 다양하고, 국제 표준인 KISA-ISMS, ISO 27001, NIST도 준수해야 한다. 사이버위기경보는 5단계로 구분되며, 각 단계별로 ACL 확인, 침해사고대응팀 소집, 네트워크 단절 검토 등 구체적 대응 절차가 정해져 있다. 탐지정책은 21개 공격유형으로 분류하고 4단계 긴급도로 구분해서 체계적으로 관리한다. 보안관제의 3대 원칙인 무중단, 전문성, 정보공유를 항상 염두에 두고, 초심을 잃지 않으면서 자기계발을 게을리하지 말아야겠다. 오늘 배운 내용은 실무 투입 시 바로 적용할 수 있는 실전 지식이다.