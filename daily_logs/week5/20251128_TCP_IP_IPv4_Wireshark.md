s### 📄 2025.11.27 (Day 24) [TCP/IP 네트워크 보안: IPv4 프로토콜 & Wireshark 패킷 분석]
...
Internet Protocol Version 4
    ...
    Time to Live: 64
    Protocol: TCP (6)
    Header Checksum: 0xb1e8 [validation disabled]
...
| 분야 | 적용 시나리오 |
|:---:|:---|
| **SOC / 관제** | **실시간 트래픽 모니터링**: IDS/IPS에서 Wireshark와 유사한 패킷 분석 엔진(Suricata, Snort)을 사용하여 비정상 패턴을 탐지합니다. **베이스라인 트래픽**을 설정하고, 통계적 이상(급증하는 SYN 패킷, 비정상 TTL, 알려지지 않은 프로토콜)을 자동 알림합니다. 예: `alert tcp any any -> $HOME_NET any (flags:S; threshold: type both, track by_src, count 100, seconds 10; msg:"Possible SYN Flood";)`. Wireshark를 통해 알림의 실제 패킷을 확인하여 **오탐(False Positive)** 여부를 판단합니다. |
| **CERT / 사고 대응** | **포렌식 패킷 분석**: 침해 사고 시 네트워크 탭(SPAN/Mirror Port)이나 방화벽에서 수집한 PCAP 파일을 Wireshark로 분석합니다. **공격 타임라인 재구성**: (1) 초기 침투 패킷 식별 (포트 스캐닝, 취약점 익스플로잇), (2) C&C 통신 추적 (특정 IP/도메인으로의 주기적 통신, 비표준 포트 사용), (3) 데이터 유출 탐지 (대용량 파일 전송, 암호화된 터널). 예: `tcp.stream eq 123`으로 특정 TCP 세션의 모든 패킷을 추적하여 공격자의 명령어와 데이터 유출 내용을 복원합니다. DNS 쿼리 분석(`dns.qry.name`)으로 DGA(Domain Generation Algorithm) 기반 멀웨어 탐지. |
| **네트워크 운영** | **성능 최적화 및 문제 해결**: Wireshark로 **패킷 손실**, **재전송**, **레이턴시** 문제를 진단합니다. `tcp.analysis.retransmission` 필터로 재전송 패킷을 찾아 네트워크 품질 문제를 식별하고, `tcp.analysis.zero_window`로 수신 버퍼 고갈 상태를 확인합니다. **서브넷 설계**: 부서별/보안 등급별로 서브넷을 분리하여 VLAN과 결합, 방화벽 규칙으로 **Micro-Segmentation** 구현. 예: DMZ(공개 서버), Internal(직원), Management(관리 장비)를 각각 다른 서브넷으로 분리하여 측면 이동을 차단합니다. |
...
* **심화 방향:**
  - **TCP/UDP 심화**: 다음 단계로 TCP의 흐름 제어, 혼잡 제어, UDP의 특성과 활용 사례를 학습할 계획입니다.
  ...
  - **IDS/IPS 룰 작성**: Suricata나 Snort 룰을 작성하여 특정 공격 패턴을 자동 탐지하는 시스템 구축. Wireshark 분석 결과를 기반으로 룰 최적화.
...
### IPv4 주소 클래스
```
Class A: 0.0.0.0     ~ 127.255.255.255  (첫 비트 0)       /8   (네트워크 126개, 호스트 16,777,214개)
Class B: 128.0.0.0   ~ 191.255.255.255  (첫 비트 10)      /16  (네트워크 16,384개, 호스트 65,534개)
Class C: 192.0.0.0   ~ 223.255.255.255  (첫 비트 110)     /24  (네트워크 2,097,152개, 호스트 254개)
