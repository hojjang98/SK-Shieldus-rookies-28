### 📄 2025.11.27 (Day 23) [Linux 시스템 보안: 로그 관리, 무결성 검사, 패키지 & 압축]
...
gzip /backup/passwd
ls -l
# passwd → passwd.gz로 변경, 용량 감소

# 압축 해제
gunzip /backup/passwd.gz
# 또는: gzip -d /backup/passwd.gz
...
bzip2 /backup/passwd
ls -l
# passwd → passwd.bz2로 변경

# bzip2 압축 해제
bunzip2 /backup/passwd.bz2
# 또는: bzip2 -d /backup/passwd.bz2

# 압축률 비교 (동일한 파일)
cp /backup/passwd /backup/passwd1
gzip /backup/passwd1
bzip2 /backup/passwd
ls -l
# bzip2가 더 작은 용량으로 압축됨

# === tar 아카이빙 실습 ===
# 여러 파일을 하나로 묶기 (압축 없음)
tar cvf /backup/bk.tar /backup/*
ls -l
# 원본 파일들이 유지되며, .tar 파일 생성
# 주의: 절대경로 사용 시 압축 해제 시 경로 주의

# 묶음 파일 내용 확인 (풀지 않고)
tar tvf /backup/bk.tar    # 상세 정보
tar tf /backup/bk.tar     # 파일명만

# 묶음 풀기
tar xvf /backup/bk.tar
ls -l
# backup/ 디렉터리 생성됨
...
# === tar + gzip 동시 실행 ===
# 압축하며 묶기
tar zcvf /backup/bk.tar.gz /backup/*
ls -l

# 내용 확인
tar tvf /backup/bk.tar.gz

# 압축 해제하며 풀기
tar xvfz /backup/bk.tar.gz
ls -l /backup/backup

# 특정 디렉터리에 압축 해제
tar xvfz /backup/bk.tar.gz -C /tmp/
```
...
| 분야 | 적용 시나리오 |
|:---:|:---|
| **SOC / 관제** | **로그 기반 실시간 탐지**: `tail -f /var/log/auth.log \| grep "Failed password" \| awk '{print $1,$2,$3,$11}' \| tee -a /var/log/failed_logins.log`를 통해 실패한 로그인 시도를 실시간으로 추출하고 별도 파일에 누적 기록합니다. 특정 IP에서 10회 이상 실패 시 자동으로 `iptables`로 차단하는 스크립트를 `cron`에 등록하여 **자동 대응 체계** 구축이 가능합니다. `who`, `w` 명령을 주기적으로 실행하여 비인가 접속을 탐지합니다. |
| **CERT / 사고 대응** | **침해 사고 시 무결성 검사 프로세스**: 사고 발생 시 가장 먼저 `debsums -s`와 `dpkg -V`를 실행하여 **변조된 바이너리를 신속히 식별**합니다. 특히 `/bin`, `/usr/bin`, `/sbin` 디렉터리의 핵심 명령어(`ls`, `ps`, `netstat` 등)가 공격자에 의해 루트킷으로 교체되었는지 확인합니다. 변조가 확인되면 `apt install --reinstall <패키지>`로 패키지를 재설치하여 원본 복구합니다. 로그 분석: `last`, `lastb`로 공격자의 로그인 타임라인을 재구성하고, `grep`으로 auth.log에서 공격자 IP와 실행 명령을 추출합니다. |
| **시스템 운영** | **자동화된 백업 및 패치 관리**: `cron`에 `0 2 * * * tar zcvf /backup/daily_$(date +\%Y\%m\%d).tar.gz /var/www/html /etc 2>&1 \| logger -t backup`을 등록하여 매일 새벽 2시 자동 백업을 수행하고, 에러는 syslog에 기록합니다. 압축된 백업 파일은 30일 이상 보관하며 주기적으로 무결성을 검증합니다. 보안 패치: `apt update && apt list --upgradable`을 주기적으로 실행하여 패치 가능한 패키지를 확인하고, 테스트 환경에서 검증 후 운영 환경에 적용합니다. 중요 패키지는 `apt-mark hold <패키지>`로 자동 업그레이드를 막아 안정성을 보장합니다. |
...
* **심화 방향:**
  ...
  - **네트워크 보안 기초**: 다음 단계로 TCP/IP 네트워크 보안 학습을 통해 패킷 분석, 방화벽 설정, 네트워크 기반 침입 탐지 시스템(NIDS) 등을 학습할 계획입니다.
