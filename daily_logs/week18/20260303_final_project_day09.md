...
```
[수집]
항공 예약·운항시스템 → SFTP/TLS 암호화 전송 (출발 24시간 전)
       ↓
[이용·보관]
CMS/ERP 업로드 → 생산팀 최소 조회 (특수식 별도 권한)
       ↓
[현장 처리]
특수식 라벨 출력 → 조리대 부착 → 기내식 박스 탑재
       ↓
[파기]
운항 종료 7일 내 자동 삭제 / 백업 30일 후 파기 / 로그 1년 보관
```
...
```
[수집]
운항·수하물 시스템 → 내부망 API 실시간 연동
       ↓
[이용·보관]
WMS 조회 (태그 중심, 연락처는 관리자 한정)
       ↓
[제공/재위탁]
하역업체 협력 (승인 절차 필요) / 지입차주 배송
       ↓
[파기]
운송 완료 14일 내 삭제 / 로그 6개월 보관
```
...
→ 결론:
두 수탁사 모두 "중앙은 강하고 현장은 무너진" 동일한 구조적 문제를 공유함.
이는 항공 관련 수탁사의 전형적인 패턴으로,
IT 부서와 현장 운영 부서 간의 보안 인식·통제 수준 격차가 핵심 원인.

컨설팅 보고서에서 이 패턴을 명확히 지적하고
"현장 중심의 보안 교육·절차 강화"를 핵심 권고사항으로 제시해야 함.
```
