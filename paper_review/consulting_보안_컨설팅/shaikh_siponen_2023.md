...
**학습 목표:**
1. 주의기반관점(ABV)이 TMT의 보안 의사결정을 어떻게 설명하는지 이해한다.
2. 침해사고 비용 → TMT 주목도 → ISRA 실시라는 인과 경로의 이론적 구조를 파악한다.
3. 경영진 보안 커뮤니케이션 전략의 이론적 기반으로 활용 가능한 프레임워크를 확보한다.
...
[연구 모델 구조]
  ...
  침해 비용          TMT 주목도         ISRA 실시
(Breach Costs) ──H2──> (TMT Attention) ──H3──> (ISRA)
       │                                           ^
       └──────────────── H1 ──────────────────────┘
       └──────────────── H4 (매개: TMT Attention) ─┘
...
가설 검증 결과의 세부 수치, 기술통계, 매개효과 분석 결과를 분석하고, 논의 섹션에서 저자들이 제시한 해석을 다룬다.
...
연구의 한계점, 후속 연구 방향, 학술적 인용 영향력, 그리고 사이버보안 거버넌스 연구의 흐름에서 이 논문이 차지하는 위치를 다룬다.
...
```
[이전 흐름]: 침해사고 이후 외부 지향 연구
  (시장 영향, 고객 대응, 위기 커뮤니케이션)
    ↓
[이 논문, 2023]: 침해사고 이후 내부 행동 연구
  (TMT 주목도 → ISRA 실시, ABV 적용)
    ↓
[후속 방향]: ISRA 이후 의사결정 및 조직학습
...
4일간의 학습을 종합해 컨설팅 역량 관점에서 이 논문을 통합 정리하고, 12편의 누적 프레임워크와의 연계를 분석한다.
...
단면 데이터, 단일 측정, ISRA 질 미포착의 세 가지 한계
    ↓
그러나 TMT 역할 실증화·반응적 ISRA 재조명이라는 학술적 기여 명확
    ↓
...
[6] Gashgari, G., Walters, R., & Wills, G. (2017). A proposed best-practice framework for information security governance. In *Proceedings of the 2nd International Conference on Internet of Things, Big Data and Security*.
...
`#보안컨설팅` `#SecurityConsulting` `#CybersecurityGovernance` `#TopManagement` `#ISRA` `#AttentionBasedView` `#PaperReview` `#SKShieldusRookies`