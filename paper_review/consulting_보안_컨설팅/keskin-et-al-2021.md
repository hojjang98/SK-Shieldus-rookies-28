...
> **Source:** Electronics (MDPI), 2021, Vol.10, No.10, p.1168 [https://doi.org/10.3390/electronics10101168]
...
**학습 목표:**  
1. C-TPRM(사이버 서드파티 리스크 관리)의 개념 구조와 비침습적 평가 방법론의 원리 이해
2. 상용 위험 점수화 도구들이 사용하는 공통 지표 체계를 컨설팅 진단 프레임워크에 적용하는 방법 습득
3. 점수화 도구 간 불일치 원인 분석을 통해 고객사 벤더 심사 체계 설계 시 표준화 과제를 조언하는 역량 확보
...
(0-100 등)  (A-F 등)
        |
        v
[벤더 리스크 판단 → 계약 결정 / 보험 언더라이팅 / 지속 모니터링]
```
...
Ghadge et al.(2020)이 공급망 사이버 위험의 전파 메커니즘과 거시적 대응 전략을 다뤘다면, 이 논문은 개별 벤더 수준에서 위험을 어떻게 측정하고 수치화할 것인가의 실무적 문제를 다룬다. Gordon & Loeb(2002)의 보안 투자 최적화 모델과도 연결되는데, 벤더 위험 점수화는 결국 어떤 벤더에 보안 요구사항을 집중할 것인가를 결정하는 자원 배분 문제이기 때문이다. Santos-Olmo et al.(2024)의 통합 리스크 분석 방법론과 비교하면, 이 논문은 내부 리스크 분석을 외부(벤더) 영역으로 확장한 사례로 이해할 수 있다.
...
탐색적 문헌 검토                  파일럿 실증 분석
(기존 C-TPRM 방법론 정리)         (동일 조직 대상 다중 도구 비교)
        |                                  |
        v                                  v
상용 C-TPRM 도구 9개 조사    →    TPRA 회원사 중 4개 도구 선정
(BitSight, FICO, RiskRecon 등)    (FICO, BitSight, RiskRecon, ComplyScore)
        |                                  |
        v                                  v
공통 지표 6개 카테고리 도출   →    University at Albany 대상 스캔 수행
        |                                  |
        v                                  v
                    결과 정규화(0-100 스케일 변환)
                            |
...
방법:
```
1. 각 도구로부터 University at Albany 대상 스캔 보고서 수집
2. 전체 점수를 0-100 범위로 정규화
   - FICO (850점 만점) → 정규화
   - BitSight (900점 만점) → 정규화
   - RiskRecon (10점 만점) → 정규화
   - ComplyScore (100점 만점) → 변환 불필요
3. 정량적 비교: 정규화된 종합 점수 비교
4. 정성적 비교: 각 보고서의 카테고리별 평가 결과 수동 비교
5. 삼각검증(Triangulation): 2명의 연구자가 독립적으로 분석 후 결과 대조
```
...
중요한 제약이 있다. 이 보고서는 비침습적 스캔에 의한 초기 점수이며, ComplyScore의 완전한 평가는 현행 표준, 법률, 보안 정책 대비 관리 및 컴플라이언스 수준을 중심으로 설문과 전화/이메일 대화를 포함한 심층 방법론으로 수행된다. 또한 특별 계약 요건의 검증은 이 평가 범위에 포함되지 않는다.
...
저자들은 점수 불일치의 원인을 세 가지로 정리한다. 첫째, 평가 방법론의 차이다. 각 도구가 동일 카테고리를 다른 세부 지표와 가중치로 측정한다. 둘째, 독점 데이터셋의 포함 여부다. BitSight처럼 독점 상업 데이터를 활용하는 도구는 공개 데이터만 사용하는 도구와 다른 결과를 낼 수 있다. 셋째, 공통 기준선 이상의 추가 위험 요소 반영 방식이 도구마다 다르다.
...
벤더 심사 프로젝트에서 위험 점수화 도구를 도입하는 고객사에게는 다음을 안내해야 한다. 복수 도구를 병행 사용하되, 도구별 평가 중심축(기술 취약점 vs. 컴플라이언스)을 이해하고 목적에 맞는 도구를 선택해야 한다. 점수가 수렴하는 항목은 실재 위험으로 우선 처리하고, 점수가 발산하는 항목은 추가 조사가 필요한 신호로 해석하는 것이 적절하다.
...
이 연구의 한계(단일 조직, 4개 도구, 파일럿 수준)가 학계에서 어떻게 평가됐는지, 그리고 이후 C-TPRM 분야가 어떤 방향으로 발전했는지 살펴본다. 또한 이 논문이 제기한 표준화 문제가 현재 산업계와 규제 기관에서 어떻게 다뤄지고 있는지도 확인한다.
...
벤더 수가 수십 개 이상이고 전수 침투 테스트나 현장 감사가 불가능한 중대형 고객사에서 1차 스크리닝 도구로 비침습적 위험 점수화 도구 도입을 검토할 때. ISMS-P 인증을 앞두고 외부자 보안 통제 체계를 구축해야 하는 고객사에서 벤더 분류 및 우선순위 설정 기준을 마련할 때.
...
5일간의 학습을 마무리하며, 이 논문이 기존 8편의 컨설팅 논문 프레임워크에 어떻게 통합되는지 정리한다. 특히 Ghadge et al.(2020)과의 연결, ISMS-P 외부자 보안 통제와의 매핑, 그리고 고객사 벤더 심사 체계 설계 시 이 논문의 발견을 어떻게 활용할 것인지를 구체적인 컨설팅 시나리오로 정리한다.
...
단일 조직 파일럿, 도구 4개, ComplyScore 평가 완성도 불일치
    ↓
인용 3건이지만 NIST SP 800-161 Rev.1(2022), CSF 2.0(2024)로 후속 표준화 진전
    ↓
