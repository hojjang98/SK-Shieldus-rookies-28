...
**학습 목표:**
1. 인식 캠페인이 행동 변화로 이어지지 못하는 심리학적 원인 구조를 이해한다
2. 보호동기이론(PMT), 조절초점이론 등 행동 변화 이론을 컨설팅 권고안 설계에 활용할 수 있는 수준으로 습득한다
3. 고객사의 보안 교육 프로그램 진단 및 개선 방향 제시 역량을 구체화한다
...
개인적 요인                    사회·환경적 요인              문화적 요인
- 지식, 기술, 경험             - 사회적 규범                 - 개인주의 vs. 집단주의
- 개인 동기 (내재적/외재적)    - 환경적 단서                 - 권력 거리
- 지각된 통제감                - 주변의 영향                 - 불확실성 회피
- 조절 초점 (촉진/예방)        - 조직 문화                   - 문화적 메시지 적합성
- 보안 피로
```
...
이 논문이 2014년에 제기한 문제들이 이후 10여 년간 어떻게 다뤄졌는지, 후속 연구에서 어떤 실증적 진전이 있었는지, 그리고 이 논문이 보안 인식 분야의 연구 흐름에 어떤 영향을 미쳤는지를 확인한다.
...
**다음 읽을 논문 방향:**
이 논문의 한계인 조직 수준 분석과 효과 측정 방법론을 보완하는 방향으로, 보안 교육 효과성 측정 또는 보안 행동 문화 정착 관련 실증 연구를 다음 탐색 대상으로 고려할 수 있다.
...
지식 전달만으로는 행동 변화가 일어나지 않는다는 문제 제기
    ↓
→ 인식(awareness)과 행동(behaviour)은 별개의 과정이며,
  그 사이에는 동기, 능력, 문화적 맥락이 개입한다
```
...
공포 소구 역효과, 측정 체계 부재, 단방향 전달 구조의 한계 확인
    ↓
→ 성공 조건 5가지: 전문적 설계, 공포 자제, 실행 가능한 교육,
  지속 피드백, 문화 맥락 반영
```
...
354회 인용, Sasse 본인의 2022년 후속 연구로 문제의식 계승
    ↓
→ 10년이 지나도 같은 문제가 반복되는 것은 기술이 아닌
  조직 관행의 문제임을 시사한다
```
...
왜 작동하는가: 메시지가 수신자의 기존 동기 구조와 일치할 때 인지적 저항이 낮아지고 수용도가 높아진다.

왜 한계가 있는가: 문화는 단일하지 않으며 조직 내에도 다양한 하위 문화가 존재한다. 국가 수준의 문화 차원이 개인 수준의 반응을 예측하는 데는 한계가 있다.

#### B. 일반화 가능한 원칙

다른 보안 컨설팅 상황에 적용 가능한 교훈:
...
유사 문제 해결에 활용 가능한 접근법: 이 논문의 심리학적 접근 방식은 보안 인식 교육뿐 아니라 보안 정책 설계, 사고 보고 문화 형성, 개인정보보호 교육 체계 설계 등 행동 변화가 목표인 모든 보안 개입에 동일하게 적용할 수 있다.
...
비즈니스 측면: 보안 교육 투자 대비 효과 개선. 형식적 이수 기록만 남는 교육 예산 낭비 방지. 임직원의 보안 행동이 실질적으로 변화하여 보안 사고 발생 빈도 감소.
...
기업 규모: 규모와 무관하게 적용 가능하나, 현실적으로는 다음과 같이 차별화된다. 대기업은 전사 규모의 보안 인식 프로그램 재설계 자문에 활용 가능하다. 중견·중소기업은 제한된 예산 내에서 효과적인 교육 체계를 설계하는 방향 자문에 활용 가능하다.

보안 성숙도: 이미 보안 교육을 실시하고 있으나 효과가 없다고 느끼는 중간 성숙도 조직에서 가장 큰 가치를 발휘한다. 교육 자체가 없는 초기 단계 조직에는 먼저 기본 체계 수립이 선행되어야 한다.

기술 스택: 특정 기술 환경에 의존하지 않는다. 피싱 시뮬레이션 플랫폼, LMS(Learning Management System) 등 교육 인프라가 있으면 효과 측정 체계 구축이 용이하다.
...
인력:
- 보안 교육 설계자 또는 외부 전문 기관 자문 필요
- 교육 효과 측정 및 분석을 담당할 내부 인력 (보안 담당자 또는 HR과 협업)

기술:
- 행동 기반 측정을 위한 피싱 시뮬레이션 도구 또는 LMS 내 행동 추적 기능
- 교육 이수율이 아닌 행동 변화를 추적할 수 있는 데이터 수집 체계
...
이 논문의 관점을 어떻게 적용할 수 있나: 고객사의 현행 보안 인식 교육 체계를 진단할 때, 이 논문의 실패 요인 6가지를 진단 체크리스트로 활용한다. 단순히 교육을 실시하고 있는지가 아니라, 교육이 실제 행동 변화를 목표로 설계되어 있는지를 확인한다.
...
어떤 보안 전략 수립에 참고할 수 있나: 디지털 전환 추진 과정에서 새로운 시스템과 프로세스를 도입할 때, 보안 행동을 사후 교육이 아닌 시스템 설계 단계에서 내재화하는 전략(Security by Design)의 이론적 근거로 활용한다. 또한 보안 문화 수립 로드맵 설계 시 인식 제고에서 행동 형성, 습관화로 이어지는 단계별 전환 구조를 설계하는 데 참고한다.

적용 예시:
- 신규 임직원 온보딩 프로세스에 보안 행동 형성 단계를 별도로 설계하여, 지식 전달(1주차) → 행동 연습(2~4주차) → 피드백 및 내재화(2~3개월) 구조로 운영
...
답변: 현재의 교육이 지식을 전달하는 데는 성공하고 있을 수 있지만, 그것만으로는 행동 변화가 보장되지 않는다. 보안 교육 심리학 연구에 따르면 사람들은 올바른 답을 알면서도 실제로 그렇게 행동하지 않는 경우가 반복적으로 확인된다. 교육이 행동 변화로 이어지려면 지식 외에도 실행 가능한 행동 지침 제시, 지속적 피드백, 반복 훈련이 함께 제공되어야 한다. 현재 피싱 클릭률을 실제로 측정하고 있다면 그것이 가장 직접적인 지표이며, 측정하고 있지 않다면 그것부터 시작해야 한다.

질문 2: 임직원들이 보안 교육을 귀찮아한다. 어떻게 해야 하는가?

답변: 보안 피로(security fatigue) 현상이다. 지나치게 많은 경고와 복잡한 지시에 반복 노출된 임직원은 보안 절차 자체를 기피하게 된다. 이를 해결하려면 두 가지가 필요하다. 첫째, 교육의 양을 줄이고 직무와 직접 관련된 내용으로 집중시켜야 한다. 둘째, 보안 행동 자체를 더 쉽게 만들어야 한다. 사람을 바꾸려는 시도보다 행동하기 쉬운 환경을 만드는 것이 장기적으로 더 효과적이다.

질문 3: 경영진에게 보안 교육 예산 증액을 설득해야 한다. 어떤 논거를 써야 하는가?

답변: 현재 교육 방식의 비효율을 먼저 수치로 보여주는 것이 효과적이다. 연간 교육 이수율은 100%에 가깝지만 피싱 클릭률이나 보안 사고 건수가 개선되지 않는다면, 현재 예산이 행동 변화가 아닌 형식적 이수 기록에만 쓰이고 있다는 근거가 된다. 여기에 피싱 시뮬레이션 기반 측정 체계나 행동 개입 중심 프로그램으로 전환할 경우 기대되는 사고 감소 효과를 손실 프레임으로 제시하면 설득력이 높아진다. 사람들은 이득보다 손실에 더 민감하게 반응한다는 심리학적 원리가 여기서도 적용된다.
...
금융: 전자금융감독규정 및 금융보안원 금융회사 정보보안 컨설팅 가이드에서 임직원 보안 교육의 주기적 실시와 효과 평가를 요구한다. 이 논문의 행동 변화 프레임워크는 금융사가 규정 요건을 형식이 아닌 실질로 이행하는 방식을 설계하는 데 적용 가능하다.

의료: 개인정보보호법 및 의료법에 따라 개인(의료)정보 취급자에 대한 정기 교육이 의무화되어 있다. 의료 현장의 높은 업무 강도와 보안 피로 위험을 고려한 맞춤형 교육 설계가 필요하다.

제조: 산업기술보호법 및 영업비밀 보호 관련 규정에서 내부자 보안 교육을 요구한다. 연구개발 인력과 생산 인력의 보안 위협 프로파일이 다르므로 직무별 맞춤 교육 설계가 특히 중요하다.
...
유사 기술 비교: KnowBe4, Proofpoint Security Awareness, Cofense 등 피싱 시뮬레이션 기반 플랫폼들은 이 논문이 제기한 행동 측정 필요성을 실무 도구로 구현한 사례다. 도입 평가 시 단순 콘텐츠 수량이나 UI보다 위 네 가지 기준으로 비교하는 것이 이 논문의 프레임워크와 일치한다.

#### C. 전문성 영역

답할 수 있는 질문:
- 왜 보안 교육을 해도 피싱 사고가 줄지 않는가
- 보안 교육의 효과성을 어떻게 측정할 것인가
- 임직원의 보안 피로를 어떻게 진단하고 완화할 것인가
- 문화적 맥락을 반영한 보안 메시지 설계 원칙은 무엇인가
- ISMS-P 심사에서 보안 교육 효과성을 어떻게 입증할 것인가

아직 답할 수 없는 질문:
- 한국 기업 문화 특성에 최적화된 보안 인식 캠페인 설계 방법론 (국내 특화 실증 연구 부족)
- 피싱 시뮬레이션 외 행동 변화를 측정할 수 있는 다양한 지표 설계 방법
- 원격 근무 환경에서 보안 행동 형성 및 유지 전략
...
보안 교육 이수율은 형식 지표다. 행동 변화는 실질 지표다. 이 두 가지를 구분하는 언어를 갖게 되면서, 고객사가 교육을 잘 하고 있다고 생각하지만 실제로는 형식만 충족하고 있는 상태를 진단하고 설명할 수 있게 되었다. 이 구분은 ISMS-P 심사 대응에서도, 경영진 설득에서도 즉시 활용할 수 있는 언어다.

**핵심 배움 2: 사람 문제가 아니라 설계 문제라는 시각**

임직원이 보안 정책을 따르지 않을 때 흔히 나오는 반응은 교육이 부족하다거나 의식이 낮다는 것이다. 이 논문은 그 전에 먼저 정책과 시스템이 사람이 따르기 쉽게 설계되어 있는지를 확인하라고 한다. 이 시각 전환은 컨설팅 진단에서 근본 원인을 찾는 방향을 바꾼다.

**핵심 배움 3: 심리학 이론이 컨설팅 논거가 된다**

PMT, EPPM, 조절초점이론은 심리학 이론이지만, 이것을 이해하면 왜 공포 강조형 교육이 역효과를 내는지, 왜 문화 맞춤형 메시지가 필요한지를 경영진에게 설명할 수 있다. 이론을 알면 경험이 없어도 논거를 만들 수 있다는 것을 이 논문에서 확인했다.
...
장기 목표:
- 6개월 후: 9편의 논문 프레임워크를 통합한 보안 컨설팅 제안서 초안 작성 가능 수준
- 1년 후: 고객사의 보안 교육 체계 진단부터 개선 방향 제시까지 독립적으로 수행 가능 수준
...
- 거버넌스, 컴플라이언스, 리스크, 개인 행동, 감사, 공급망,
  투자, 보안 문화, 보안 인식 및 행동 변화 9편 완료

Phase 2 (진행 중): 연결
- 9편의 논문이 거버넌스-리스크-컴플라이언스-투자-문화-행동의
  층위 구조로 연결됨을 확인
...
단순한 기술 이해자가 아닌:
- 행동 변화의 원리를 설명할 수 있는 컨설턴트
- 고객사의 교육 형식 뒤에 있는 설계 실패를 진단할 수 있는 자문가
- 심리학 이론과 비즈니스 문제를 연결할 수 있는 전문가
...
[4] Witte, K. (1994). Fear control and danger control: A test of the Extended Parallel Process Model (EPPM). *Communication Monographs*, 61, 113–134.

[5] Dolan, P., Hallsworth, M., Halpern, D., King, D., & Vlaev, I. (2010). MINDSPACE: Influencing behaviour through public policy. Institute for Government, Cabinet Office.

[6] Hofstede, G., Hofstede, J. G., & Minkov, M. (2010). *Cultures and Organizations: Software of the Mind* (3rd ed.). McGraw-Hill.

[7] Sasse, M. A., Hielscher, J., Friedauer, J., & Buckmann, A. (2023). Rebooting IT Security Awareness: How Organisations Can Encourage and Sustain Secure Behaviours. In *Computer Security. ESORICS 2022 International Workshops*. Lecture Notes in Computer Science, vol 13785. Springer.

[8] Da Veiga, A., & Eloff, J. H. P. (2010). A framework and assessment instrument for information security culture. *Computers & Security*, 29(2), 196–207.
