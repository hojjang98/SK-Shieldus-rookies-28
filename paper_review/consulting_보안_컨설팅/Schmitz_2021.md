# Research Review: Maturity Level Assessments of Information Security Controls: An Empirical Analysis of Practitioners' Assessment Capabilities
> **Analyzed Date:** 2026.04.14 - 2026.04.18
...
# Research Review: Maturity Level Assessments of Information Security Controls
...
│                                             │
│  ISO/IEC 27002 통제 선별                    │
│  (정책 A.5 / 물리보안 A.11 / 취약점 A.12)   │
│          ↓                                  │
│  BSI IT-Grundschutz 조치 매핑               │
│          ↓                                  │
│  각 통제별 시나리오 성숙도 수준(SML) 정의    │
│  (상위·하위 경계 명시)                       │
│          ↓                                  │
│  가상 기업 시나리오 구성 (CloudSec)          │
│  + 전문가 2라운드 사전 검증                  │
└─────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────┐
...
│                  │                          │
│ - 선형 편차 측정 │ - 개방형 응답 코딩        │
│ - Shapiro-Wilk  │   (시나리오 오해석,       │
│   정규성 검정   │    통제 오해석,           │
│ - t검정 (집단간) │    과도한 조치 제안)      │
│ - Spearman 상관 │ - 인터뷰 전사 코딩        │
└──────────────────┴──────────────────────────┘
```

**설계 철학:**
기존 성숙도 평가 신뢰성 연구들은 평가자 간 일치도(inter-rater reliability)만 측정했다. 평가자들이 서로 비슷하게 틀려도 신뢰도는 높게 나올 수 있다는 구조적 한계가 있다. 이 연구는 정답이 사전에 정의된 시나리오를 설계함으로써, 일치도가 아닌 타당성(validity) — 즉 실제 정답과의 편차 — 을 측정할 수 있는 구조를 만들었다.
...
개방형 응답은 두 코더가 독립 코딩 후 합의하는 방식으로 분석되었다. 코드 체계는 다음과 같다.
...
**Day 3 Preview:**
정량·정성 분석의 실제 결과를 살펴본다. 어떤 통제 항목에서 편차가 가장 컸는지, 어떤 전문성 요인이 통계적으로 유의미했는지, 그리고 던닝-크루거 패턴이 어떤 집단에서 어떻게 나타났는지 구체적인 수치와 함께 분석한다.
...
**Day 4 Preview:**
이 연구의 한계점과 함께, 성숙도 평가 품질 연구가 학계와 산업에 미친 영향을 살펴본다. 특히 독일 자동차 산업의 VDA-ISA 맥락과 이 연구의 발견이 실무 표준화 논의에 어떻게 연결되는지 확인한다.
...
**Day 5 Preview:**
지금까지 13편의 논문을 통해 쌓아온 보안 컨설팅 지식 레이어를 이 논문과 통합한다. 거버넌스-컴플라이언스-리스크-개인행동-감사-공급망-투자-문화-인식-제3자리스크-사고대응-경영진주의-성숙도평가라는 누적 프레임워크를 완성한다.
...
가상 시나리오·독일 맥락·56명 표본이라는 제약
    ↓
그러나 전용 훈련 과정 부재라는 산업 공백을 정면으로 드러냄
    ↓
...
```
Phase 1 (완료): 논문 이해
- 14편의 논문을 통해 거버넌스·리스크·컴플라이언스·
  인간 요인·성숙도 평가의 학술적 기반 확보
...
[1] Schmitz, C., Schmid, M., Harborth, D., & Pape, S. (2021). Maturity level assessments of information security controls: An empirical analysis of practitioners' assessment capabilities. *Computers & Security*, 108, 102306. https://doi.org/10.1016/j.cose.2021.102306

[2] Foorthuis, R., & Bos, R. (2011). A framework for compliance management. *Journal of Information System Security*, 7(2).

[3] Slapničar, S., Vuko, T., Čular, M., & Drašček, M. (2022). Effectiveness of cybersecurity audit. *Managerial Auditing Journal*, 37(8), 996-1021.
...
[5] El Emam, K., Drouin, J., & Melo, W. (Eds.). (1998). *SPICE: The theory and practice of software process improvement and capability determination*. IEEE Computer Society Press.
