...
미안! 양식을 칼같이 맞추는 게 컨설턴트의 기본인데 내가 잠시 놓쳤네. 호준이가 정리하던 **Day 1, Day 2의 톤앤매너와 카테고리 양식**을 그대로 이어받아서 **Day 3**를 작성해 줄게.

---

# Day 3 - Empirical Evidence & Limitations

*(1/e 법칙의 실무적 적용과 모델의 한계)*

---

## 1. 연구 결과: 보안 투자의 '마법의 숫자' 37%

**1/e 규칙의 도출**

논문의 가장 파격적인 결론은 보안 투자의 경제적 상한선을 제시한 것이다. 저자들은 수학적 증명을 통해 최적 보안 투자액($z^*$)이 침해로 인한 기대 손실($vL$)의 **$1/e$ (약 36.79%)**를 초과하지 않아야 함을 밝혀냈다. 이는 보안 예산이 잠재적 손실액의 약 1/3을 넘어서는 순간, 추가 투자로 얻는 보안 강화 효과보다 투입되는 비용이 더 커지는 '비효율 구간'에 진입함을 의미한다.

**취약성 수준에 따른 차별적 전략**

특히 Class II 함수(취약성에 대해 엄격히 오목한 함수) 모델은 모든 자산에 동일하게 투자하는 것이 비효율적임을 시사한다.

* **저취약성 자산:** 이미 충분히 안전하므로 추가 투자의 경제성이 낮음.
* **고취약성 자산:** 보호를 위한 한계 비용이 너무 높아 투자가 비합리적임. (위험 수용 또는 보험 고려)
* **중취약성 자산:** 보안 투자 효율이 가장 극대화되는 구간으로, 기업의 자원을 집중해야 할 대상.
...
| 한계 항목 | 내용 | 비고 |
| --- | --- | --- |
| **수치화의 어려움** | 위협 확률($t$)과 취약성($v$)을 정확한 상수로 도출하는 방법론 부재 | 실무 적용 시 주관 개입 가능성 |
| **위험 중립성 가정** | 재앙적 손실(기업 파산 등)에 대한 '위험 회피' 성향을 반영하지 않음 | 핵심 기반 시설 등에는 적용 제한 |
| **단일 기간 모델** | 보안 투자의 장기적 효과나 유지보수 비용 등 시계열적 특성 미반영 | 단기적 의사결정에 편중될 위험 |
| **상호 의존성 무시** | 한 정보 집합의 보안이 다른 집합에 미치는 영향(연쇄 반응) 고려 부족 | 현대의 복합적인 IT 환경 반영 미흡 |

---

## 3. 이론적 확장: 2002년 이후의 변화

```
Gordon-Loeb (2002) 기본 모델
    [1/e 상한선 제시]
         |
         v
위험 회피(Risk-Averse) 모델 확장
    - 손실에 민감한 기업은 1/e 이상 투자 정당화 가능
         |
         v
다수 위협 및 상호의존성 모델
    - 네트워크로 연결된 자산 간의 리스크 전이 반영
         |
         v
현대적 ROSI(Return on Security Investment)
    - Gordon-Loeb의 ENBIS 개념을 실무 지표로 구체화
...
이 논문은 고객사의 보안 예산 적정성을 검토할 때 가장 강력한 **'벤치마크 가이드라인'**이 된다. 고객사가 "왜 우리 예산이 이 정도여야 합니까?"라고 물을 때, 단순한 감이 아니라 학술적으로 검증된 $1/e$ 규칙을 인용하며 "예상 손실의 37%가 경제적 최적점"이라는 논리를 펼칠 수 있다. 또한, 모든 자산을 방어하려는 고객에게 **'선택과 집중(중간 취약성 자산 타겟팅)'**의 필요성을 경제학적으로 설득하는 도구가 된다.

**기존 학습과의 연결:**

| 기존 논문 | 연결 방식 |
| --- | --- |
| **Santos-Olmo (2024)** | MARISMA를 통해 측정된 리스크 수치를 Gordon-Loeb 모델의 $L$값으로 대입하여 구체적 예산안 도출 가능 |
| **Gashgari (2017)** | 이사회가 자원 배분 시 "왜 전액 투자가 아닌 일부 투자가 최적인가"를 설명하는 리포트 작성의 근거 |
| **실무 프로젝트** | SK 쉴더스 루키즈 과정 중 수행하는 리스크 분석 프로젝트에서 '투입 대비 효과'를 정량화하는 프레임워크로 활용 |

**현실적 고려사항:**

컨설턴트는 이 모델의 **'정직한 전달자'**가 되어야 한다. 1/e는 절대적인 법칙이 아니라 '경제적 가이드라인'임을 명시해야 한다. 특히 개인정보 유출이나 징벌적 손해배상이 걸린 사안에서는 기업이 위험 중립적일 수 없으므로, Gordon-Loeb 모델을 베이스라인으로 삼되 기업의 위험 성향에 따라 가중치를 조절하는 유연함이 필요하다.