...
보안 침해 확률 함수 S(z, v) 정의
    ...
    A3: Sz < 0, Szz > 0  [투자 증가 시 침해 확률 감소, 단 수확 체감]
         |
         v
기대 편익 EBIS(z) = [v - S(z,v)] * L
기대 순편익 ENBIS(z) = [v - S(z,v)] * L - z
         |
         v
최적화 조건 (1차 조건)
    -Sz(z*, v) * L = 1
...
**학술적 기여:**
- 정보보안 투자를 경제 최적화 문제로 처음 정형화한 논문으로, 보안 경제학이라는 연구 분야의 출발점을 만들었다.
- 취약성과 최적 투자액의 관계가 단조 증가가 아닐 수 있다는 것을 두 가지 보안 침해 확률 함수 클래스를 통해 수학적으로 증명했다. 이는 기존의 직관적 가정(취약할수록 더 투자해야 한다)을 반박하는 것이다.
- 최적 투자액의 상한이 예상 손실의 1/e임을 두 가지 함수 클래스 모두에서 도출하여, 실무에서 참조할 수 있는 구체적 수치 기준을 처음으로 제시했다.
...
이 논문의 가장 직접적인 컨설팅 활용 가치는 고객 경영진과의 대화에서 드러난다. 보안 투자에 얼마를 써야 하는가라는 질문에 대해 막연히 많이 써야 한다거나 업계 평균을 따르라고 답하는 대신, 예상 손실 L을 추정하고 그 37% 이하를 투자 상한선으로 제시하는 구체적 논거를 갖추게 된다. 더 중요하게는, 투자액을 무작정 늘리는 것이 최적이 아니라는 논거, 즉 수확 체감의 법칙이 보안에도 적용된다는 점을 경제 모델로 설명할 수 있게 된다.
...
Class II에서 -αv ln v > 0은 0 < v < 1 구간에서 성립하며, v = 1/e ≈ 0.3679에서 최대값을 가진다. 주어진 L에 대해 1/L > -αv ln v를 만족하는 v 구간(극단적으로 낮거나 높은 취약성)에서는 최적 투자액이 0이 된다. 논문은 α = 0.00001, L = 400,000달러인 경우 V ≈ 0.1, V ≈ 0.7로 수치 예시를 제시한다. 즉 취약성이 10% 미만이거나 70% 초과인 정보 집합에는 투자가 경제적으로 정당화되지 않는다.
...
Day 3에서는 논문의 실증적 함의, 즉 이 모델이 실제 기업의 보안 투자 결정과 얼마나 일치하는지, 그리고 1/e 규칙이 실무에서 어떻게 해석되고 활용되어 왔는지를 다룬다.
...
| 명제 | 결과 | 핵심 함의 |
|------|------|-----------|
| 명제 1 | A1-A3 만족 시 취약성 증가 구간에서 최적 투자 증가 구간 존재 | 취약성과 투자 간 최소한의 양의 관계는 항상 성립 |
| 명제 2 | Class II에서 최적 투자가 취약성에 단조 증가하지 않는 반례 존재 | 취약할수록 더 투자해야 한다는 직관이 반드시 옳지 않음 |
| 명제 3 | Class I과 Class II 모두에서 z*(v) < (1/e) * vL | 최적 투자액은 예상 손실의 37%를 초과하지 않음 |

### B. 1/e 규칙의 수학적 도출 과정

**Class I에서의 증명:**

z_I*(v)/vL을 x = αvL로 치환하면 다음 형태가 된다.

```
z_I*(v)/vL = [(βx)^(1/(β+1)) - 1] / x
```

이를 x에 대해 최대화하면 최대값은 (β/(β+1))^(β+1)이다. β → ∞의 극한에서 L'Hospital의 정리를 적용하면 이 값은 1/e로 수렴한다. β ≥ 1인 모든 유한한 β에서 이 값은 1/e 미만이므로, Class I의 모든 함수에 대해 z*(v) < (1/e) * vL이 성립한다.

β = 1인 특수 경우, 최대 비율은 25%이며 αvL = 4일 때만 이 값에 도달한다.

**Class II에서의 증명:**

z_II*(v)/vL을 x = -αvL ln v로 치환하면 다음 형태가 된다.

```
z_II*(v)/vL = ln(1/x) / (-x)
```

이 함수의 최대값을 구하면 x = e에서 최대값 1/e를 가진다. 따라서 Class II에서도 z*(v)/vL < 1/e이 성립한다.
...
취약성(v) 0 ─────────────────────────────► 1
          ...
          z*(v)=0  |  z*(v) 단조 증가
              ...
              임계값(1/αβL)
```
임계값 이하에서는 투자가 0이고, 임계값을 넘으면 취약성 증가에 따라 최적 투자액이 단조 증가한다. 직관과 일치하는 패턴이다.
...
취약성(v) 0 ─────────────────────────────► 1
    ...
    z*(v)=0   | z*(v) > 0     |  z*(v)=0
             ...
             V(L)           V̄(L)
```
낮은 취약성 구간과 높은 취약성 구간 모두에서 최적 투자액이 0이고, 중간 구간에서만 양의 투자가 정당화된다. α = 0.00001, L = 400,000달러인 예시에서 V ≈ 0.1, V̄ ≈ 0.7이다.

### D. 극단적 고취약성 자산에 투자가 정당화되지 않는 이유

Class II에서 v가 1에 가까워질수록, 즉 정보가 거의 공개 정보에 가까워질수록 침해 확률을 의미있게 낮추는 데 드는 비용이 폭발적으로 증가한다. 논문이 제시하는 예시가 명확하다. 기업이 특정 사업 부문을 매각하려 한다는 정보가 이미 거의 공공연히 알려진 상황에서, 직원과 거래처를 감시하여 정보 유출을 방지하려는 시도는 비용 대비 효과가 극히 낮다. 이미 다수의 경로로 유출 가능한 정보를 통제하는 것은 경제적으로 정당화되기 어렵다.
...
| 명제 | 검증 방법 | 결과 | 비고 |
|------|-----------|------|------|
| 명제 1 | 수학적 증명 (부록) | 성립 | A1-A3 만족 시 일반적으로 성립 |
| 명제 2 | Class II 반례 제시 | 성립 | 단조 증가가 필연적이지 않음을 증명 |
| 명제 3 | Class I, II 각각 수학적 증명 | 성립 | 두 클래스 모두 1/e 상한 확인 |
...
명제 3의 결론을 실무 맥락으로 번역하면 다음과 같다. 어떤 정보 집합의 예상 손실(L = 위협 확률 × 침해 시 손실액)이 1억 원이라면, 그 정보를 보호하기 위한 보안 투자의 경제적 상한은 약 3,679만 원이다. 이를 초과하는 투자는 기대 순편익 관점에서 최적이 아니다.

더 중요한 함의는 37%가 상한이지 권장값이 아니라는 점이다. 논문은 대부분의 경우 최적 투자액이 37%보다 훨씬 낮다고 명시한다. β = 1인 Class I 함수에서 최대 비율은 25%이며, 특정 조건에서는 훨씬 낮아진다.
...
논문이 명시한 한계 중 가장 실무적으로 중요한 것은 위협 확률과 취약성을 실제로 수치화하는 절차가 모델 범위 밖이라는 점이다. L, t, v를 추정하지 못하면 모델을 직접 적용할 수 없다. 이 공백은 이후 수많은 후속 연구의 출발점이 되었다.
...
가장 인상적인 부분은 명제 2다. 취약할수록 더 투자해야 한다는 직관이 수학적으로 반드시 옳지 않음을 Class II가 증명한다. 보안 컨설팅 현장에서 취약점 진단 결과를 가지고 고객과 대화할 때, 가장 취약한 자산이 곧 가장 많은 투자가 필요한 자산은 아닐 수 있다는 논거를 이 모델에서 얻을 수 있다. 투자 우선순위 결정에서 취약성 수치 하나만 보는 것이 아니라 보호 비용 대비 기대 편익을 함께 고려해야 한다는 원칙이 이론적으로 뒷받침된다.

**실무 이해: 37% 규칙의 양면성**

37% 규칙은 강력한 커뮤니케이션 도구이지만, 오해될 위험도 크다. 예상 손실의 37%를 써야 한다가 아니라 37%를 초과해서는 안 된다는 의미이며, 실제 최적값은 대부분 그보다 훨씬 낮다는 점을 항상 함께 전달해야 한다.
...
이 모델의 다섯 가지 명시적 한계(단일 기간, 단일 위협, 위험 중립, 수치화 문제, 복수 정보 집합 미고려)가 이후 연구 흐름을 어떻게 형성했는지, 그리고 1,304회 인용이 보여주는 학술적 영향력을 분석한다.
...
**문제:**
모든 의사결정과 결과가 동시에 발생하는 1기간 모델이다. 보안 투자의 시간 가치, 선점 우위, 동적 위협 환경에서의 적응적 투자 전략을 반영하지 못한다.

**영향:**
실제 기업의 보안 투자는 다년도 예산 주기로 이루어지며, 위협 환경도 지속적으로 변한다. 1기간 모델은 이러한 동적 특성을 포착하지 못해 장기 보안 전략 수립에 직접 적용하기 어렵다.

**보완 방향:**
논문 자체가 단일 기간 모델을 동적 모델로 확장하는 것이 흥미로운 연구 방향임을 명시했다. 이후 Krutilla et al.(2021)이 Gordon-Loeb 모델의 동적 확장을 시도했다.

### B. 단일 위협 가정

**문제:**
하나의 정보 집합에 하나의 위협만 존재한다고 가정한다. 복수 위협이 존재하는 경우 기대 손실 계산이 훨씬 복잡해지며, 위협 간 상호작용(하나의 침해가 다른 침해를 유발하는 연쇄 효과)도 고려되지 않는다.

**영향:**
현실의 보안 환경에서 조직은 동시에 다수의 위협(외부 해커, 내부자, 자연재해, 공급망 등)에 노출되어 있다. 단일 위협 가정은 모델의 적용 범위를 제한한다.

**보완 방향:**
복수 위협 모델은 복잡성이 크게 증가하지만, 게임 이론 접근법을 통해 공격자-방어자 간 전략적 상호작용을 모델링하는 연구들이 이를 부분적으로 보완했다.

### C. 위험 중립 가정

**문제:**
기업이 동일한 기대값을 가진 투자 중 리스크 수준과 무관하게 선택한다고 가정한다. 논문 자체가 재앙적 손실 시나리오에서는 위험 회피 가정이 더 현실적임을 인정한다.

**영향:**
현실의 경영진은 분산이 큰 결과를 회피하는 경향이 있다. 특히 대규모 개인정보 유출, 핵심 인프라 마비 등 재앙적 결과를 수반하는 보안 사고에서는 기대값 기반 의사결정이 경영진의 실제 선호를 반영하지 못한다. 위험 회피 정도가 높을수록 최적 투자액은 이 모델이 제시하는 수준보다 높아진다.

**보완 방향:**
위험 회피 유틸리티 함수를 도입한 확장 모델이 후속 연구에서 제시되었다. 다만 이 경우 의사결정자의 구체적인 위험 회피 성향(유틸리티 함수의 형태)을 추정해야 하는 추가적 어려움이 생긴다.

### D. 위협 확률과 취약성의 수치화 문제

**문제:**
모델이 입력값으로 요구하는 t(위협 확률), v(취약성), λ(침해 시 손실액)를 실제로 어떻게 추정하는가에 대한 방법론이 모델 범위 밖에 있다.

**영향:**
이것이 이론과 실무 사이의 가장 큰 간극이다. 모델을 실제 기업에 적용하려면 이 수치들을 신뢰할 수 있는 방법으로 추정해야 하지만, 보안 침해는 드물게 발생하고 관련 데이터는 공개되지 않는 경우가 많아 통계적 추정이 어렵다.

**보완 방향:**
이후 사이버 보험 시장의 발전, CVE 데이터베이스 활용, 침해 사고 데이터 수집 노력(Verizon DBIR 등)이 이 공백을 부분적으로 채우는 시도들이다.

### E. 복수 정보 집합 간 상관관계 미고려

**문제:**
단일 투자가 상호 연관된 리스크를 가진 복수의 정보 집합을 동시에 보호하는 경우를 다루지 않는다. 실제 보안 투자(예: 방화벽, 암호화 인프라)는 여러 자산을 동시에 보호하는 특성이 있다.

**영향:**
자산 간 보안 리스크의 상관관계를 무시하면 투자 효율성이 과소 또는 과대 평가될 수 있다. 또한 보안 투자와 비정보 자산(인력, 시설) 보호 투자 간의 최적 배분 문제도 다루지 못한다.
...
- Wall Street Journal, Financial Times 등 주류 언론에서 다뤄진 보안 경제학 분야의 유일한 논문 중 하나
- 2004년 단행본 Economics of Information Security에 재수록

이 인용 수는 동일 분야 Slapničar(2022) 약 90회, Ghadge(2020) 약 115회와 비교할 때 압도적이다. 보안 경제학이라는 분야 자체를 창시한 논문이기 때문이다.

### B. 연구 트렌드의 변화

```
[이 논문 이전 2002년까지]: 기술적 방어와 행동적 측면 중심
    보안 = 기술 문제 또는 인간 행동 문제
    투자 결정 = 직관 또는 업계 관행 의존
         |
         v
[이 논문 2002년]: 보안 경제학의 출발
    보안 = 경제적 최적화 문제
    투자 결정 = 기대 순편익 최대화
    핵심 기여: 1/e 규칙, 중간 취약성 집중 전략
         |
         v
[이후 연구 흐름]:
    - 게임 이론 확장 (공격자-방어자 전략적 상호작용)
    - 동적 모델 확장 (다기간, 적응적 투자)
    - 위험 회피 확장 (유틸리티 기반 모델)
    - 네트워크 외부효과 포함 (상호연결 조직 간 보안 투자)
    - 사이버 보험과 보안 투자의 결합 모델
    - 실증 검증 시도 (기업 데이터, 침해 사고 데이터 활용)
```

### C. 주요 후속 연구 방향

**게임 이론 확장:**
단일 의사결정자 모델의 한계를 극복하여 공격자와 방어자 간의 전략적 상호작용을 모델링했다. 논문 자체도 이 방향을 명시적으로 제안했으며, Jajodia & Millen(1993)의 컴퓨터 보안은 설계자와 잠재 공격자 간의 게임이라는 관점을 이론적으로 발전시켰다.

**네트워크 외부효과 포함:**
Dey et al.(2012)은 상호연결된 조직들 사이에서 한 조직의 보안 투자가 다른 조직에 미치는 간접 효과를 Gordon-Loeb 모델에 통합했다. 이는 Ghadge(2020)의 공급망 사이버 리스크 연구와 연결되는 흐름이다.

**사이버 보험과의 통합:**
보안 투자와 사이버 보험을 결합한 혼합 리스크 관리 전략의 최적화 모델들이 개발되었다. 보험료가 보안 투자 수준에 따라 감소하는 구조를 반영하여 두 수단 간 최적 배분을 다룬다.

**실증 검증:**
논문 자체가 향후 연구 과제로 명시한 방향이다. 기업들이 실제로 이 모델과 일치하는 방식으로 보안에 투자하는지, 일치하지 않는다면 그 원인이 무엇인지를 실증 데이터로 검증하는 연구들이 이어졌다. Weishäupl et al.(2018)의 다중 사례 연구가 대표적이며, 실제 기업들은 표준화된 투자 결정 프로세스 없이 주로 외부 환경에 반응적으로 투자한다는 결과를 보였다.
...
이 논문 이전에는 보안 투자 결정이 주로 직관, 업계 관행, 규제 최소 요건 충족에 의존했다. 이 논문 이후 보안을 경제적 최적화 문제로 바라보는 시각이 학계와 실무 모두에서 자리잡기 시작했다. 보안 솔루션 업체들이 ROI를 강조하고, CISO가 경영진에게 보안 투자를 비용-편익 언어로 설명하는 현재의 관행은 이 논문이 만든 담론 구조 위에 있다.

### B. 사이버 보험 산업에 미친 영향

보안 침해 확률 함수라는 개념은 사이버 보험 프리미엄 산정의 이론적 기반이 되었다. 보험사가 피보험 기업의 취약성 수준과 잠재 손실을 평가하여 보험료를 책정하는 방식은 Gordon-Loeb 모델의 파라미터 체계와 직접적으로 연결된다.

### C. CISO의 경영진 보고 언어 변화

이 논문 이후 CISO가 이사회에 보안 투자를 보고하는 방식이 변화했다. 기술적 위협 목록 나열에서 예상 손실, 투자 효과, 기대 순편익 중심의 재무적 언어로의 전환이 이루어지기 시작했다. Gashgari(2017)가 다룬 정보보안 거버넌스 프레임워크에서 이사회의 역할이 강조되는 것도 이러한 맥락과 연결된다.
...
이 논문의 한계를 아는 컨설턴트는 두 가지를 할 수 있다. 첫째, 모델을 정확한 계산 도구가 아닌 사고 프레임워크로 제시할 수 있다. 정확한 숫자를 구하기 어렵더라도, 예상 손실의 37% 이하라는 상한선은 과도한 보안 지출을 경고하는 데 유용하다. 둘째, 위험 중립 가정의 한계를 솔직히 전달하면서 고객사의 위험 회피 성향에 따라 이 모델이 제시하는 수준을 조정해야 함을 안내할 수 있다.

**적용 가능 시나리오:**
- 보안 예산 상한선 검토: 고객사가 예상 손실 대비 37%를 초과하는 보안 지출을 계획하고 있다면, 경제적 최적성 관점에서 재검토를 권고하는 근거로 활용 가능
- 투자 우선순위 결정: 여러 자산 중 어디에 먼저 투자할지 결정할 때, 취약성뿐 아니라 보호 비용 대비 기대 편익을 고려하는 프레임워크로 활용

**적용 불가 시나리오:**
- 재앙적 손실 가능성이 있는 환경: 핵발전소, 금융 결제 시스템, 국가 핵심 인프라 등에서는 위험 중립 가정이 성립하지 않아 이 모델을 직접 적용하기 어렵다
- 수치화가 불가능한 환경: 위협 확률과 취약성을 합리적으로 추정할 수 없는 경우, 모델의 수식보다 그 사고 방식(한계편익 = 한계비용)만 차용하는 것이 현실적이다
...
이 논문이 5가지 한계를 결론 섹션에서 명시적으로 나열한 것은 약점이 아니라 강점이다. 한계를 명확히 정의함으로써 후속 연구의 지도를 그렸고, 그 지도 위에서 1,304편의 후속 연구가 쌓였다. Slapničar(2022)가 감사 효과성 연구에서 H2를 기각했을 때 그것이 오히려 솔직한 연구의 가치를 보여주었던 것처럼, 이 논문도 자신의 한계를 솔직히 인정함으로써 학술적 신뢰를 얻었다.

**Trade-off 이해:**

1/e 규칙이 갖는 핵심 Trade-off는 단순성과 적용 가능성 사이의 것이다. 모델이 단순할수록 통찰이 명확하지만 현실 적용성이 낮아지고, 복잡해질수록 현실을 잘 반영하지만 핵심 메시지가 흐려진다. Gordon & Loeb는 단순성을 선택하여 명확한 메시지(37% 상한, 중간 취약성 집중)를 얻었다. 컨설팅에서도 같은 원칙이 적용된다. 고객에게 전달할 핵심 메시지를 단순하게 만드는 대신, 그 단순성의 가정과 한계를 함께 전달하는 것이 좋은 자문이다.

**다음 읽을 논문 방향:**

이 모델의 핵심 공백인 위협 확률과 취약성의 수치화 문제, 그리고 위험 중립 가정의 현실성을 보완하는 방향의 연구가 자연스러운 다음 단계다. 또한 이 모델이 실제 기업 투자 결정과 얼마나 일치하는지를 실증적으로 검증한 Weishäupl et al.(2018)도 보완적으로 읽을 가치가 있다.

**Day 5 Preview:**

5일간의 학습을 보안 컨설팅 관점에서 통합한다. Gordon-Loeb 모델이 앞선 6편의 논문과 어떻게 연결되며, 경영진 설득과 보안 투자 자문 역량에 구체적으로 어떻게 기여하는지를 정리한다.
...
- 경제적 관점에서 보안의 목표는 완전한 보안이 아니라 최적 보안이다. 이 두 개념은 다르다.
...
답변: 취약성 수치만으로 우선순위를 정하는 것은 불완전하다. 극단적으로 취약한 자산은 보호 비용이 지나치게 높아 경제적으로 정당화되지 않을 수 있다. 각 자산의 침해 시 예상 손실과 취약성 개선을 위한 비용을 함께 평가하여, 보호 비용 대비 기대 편익이 높은 중간 수준 취약성 자산에 먼저 집중하는 것이 경제적으로 합리적이다.
...
**이 논문을 읽은 후:**
예상 손실이라는 개념을 중심으로 보안 투자 결정을 재구성할 수 있게 되었다. 37% 상한이라는 구체적 수치, 중간 취약성 집중 원칙, 극단적 취약 자산의 대안적 처리 방식을 논리적으로 설명할 수 있다.
...
논문이 5가지 한계를 명시적으로 인정한 것처럼, 컨설턴트도 자신이 사용하는 프레임워크의 적용 범위와 한계를 고객에게 솔직하게 전달해야 한다. 이 모델은 정밀한 계산 도구가 아니라 사고 프레임워크로 제시되어야 하며, 위험 중립 가정이 성립하지 않는 상황에서는 그 점을 명확히 해야 한다. Slapničar(2022)가 보안 감사가 공격을 예방하지 않을 수 있다는 불편한 진실을 제시했던 것처럼, 이 논문도 보안 지출을 늘리는 것이 항상 최선이 아님을 말한다.
...
**학술적 의의:**
보안을 경제적 최적화 문제로 처음 정형화하여 보안 경제학이라는 연구 분야를 창시했다. 1,304회 인용과 수십 년에 걸친 후속 연구 흐름이 이를 증명한다. 두 가지 독립적인 함수 클래스에서 동일한 상한(1/e)을 도출한 수학적 강건성이 이 논문의 장기적 영향력의 토대다.
...
Phase 2 (진행 중): 연결
- 7편이 거버넌스-컴플라이언스-리스크-행동-감사-공급망-투자라는
  보안 컨설팅의 전체 사이클을 이론적으로 커버함을 이해
...
Phase 4 (목표): 전문성
- 기술 지식과 경제적 논리를 결합하여 경영진에게 설득력 있는
  보안 투자 자문을 제공하는 컨설턴트
```
...
## Tags
`#보안컨설팅` `#SecurityConsulting` `#SecurityInvestment` `#GordonLoeb` `#ROSI` `#CybersecurityEconomics` `#PaperReview` `#SKShieldusRookies`