...
초기 9,493건            + QDA Miner 텍스트 마이닝       ↓
                        + 연결성 기반 클러스터 분석    통합 개념 모델
                        + 덴드로그램 분석               (IT보안 - 조직보안
                                                        - 공급망보안)
```
...
| 가정 | 내용 | 근거 |
|------|------|------|
| **A1** | 공급망 사이버 리스크는 단일 조직 내부의 IT 보안 문제와 구별되는 독자적 연구 영역이다 | 기존 연구가 기업 내부 관점에만 집중하여 조직 간 경계를 가로지르는 위협을 다루지 않음 |
| **A2** | 텍스트 마이닝 기반 클러스터 분석이 연구자의 주관적 코딩보다 편향이 적은 주제 분류를 가능하게 한다 | Webster & Watson(2002)의 개념 중심 접근법과 Tranfield et al.(2003)의 SLR 방법론 결합 |
| **A3** | 사이버 리스크는 기술적 위협뿐 아니라 인적·물리적 차원을 포함하는 다층적 현상이다 | 기존 분류 체계가 의도적 공격자 행위에만 초점을 맞춰 비의도적 리스크를 배제한 한계 |
| **A4** | 공급망에서 사이버 리스크는 발생 지점에서 파트너를 거쳐 사회 전반으로 전파되는 연쇄적 특성을 가진다 | 공급망 연결성의 본질적 특성 - 모든 노드와 연결이 잠재적 위협 경로가 됨 |
...
| 소스 | 수집 정보 | 용도 |
|------|-----------|------|
| Scopus | 6,637건 초기 검색 결과 | 학술 논문 광범위 수집 |
| ProQuest | 2,856건 초기 검색 결과 | 경영·사회과학 분야 보완 |
...
└──────────┬──────────┬──────────┬──────────────┬────────────────┘
           │          │          │              │
    사이버    사이버     사이버     사이버보안        완화
    리스크    리스크     리스크     과제             방안
    유형      전파       PoPs
           │          │          │              │
  - 물리적   - 1차     - 기술적   - 협업          - 공격 전
    위협      전파       수준       (Collaboration)  (Pre-attack)
  - 시스템   - 2차     - 인적     - 직원 지식      - 공격 중
    고장      전파       수준       (Employee         (Trans-attack)
  - 간접     - 3차     - 물리적     knowledge)      - 공격 후
    공격      전파       수준     - 지속적            (Post-attack)
  - 직접                          헌신
    공격                          (Continuous
  - 내부자                         commitment)
    위협                        - 정부 관여
...
| 도구/방법 | 접근 방식 | 강점 | 약점 |
|-----------|-----------|------|------|
| 기존 VA/PT | 단일 조직 내부 취약점 식별 | 기술적 취약점 상세 발견 | 파트너 간 경계 위협 미탐지 |
| ISMS-P 인증 | 조직 내부 보안 통제 체계 검증 | 규제 준수 달성 | 공급업체 보안 수준 검증 미흡 |
| Ghadge et al.(2020) | 공급망 전체 사이버 리스크 지형 분류 | 조직 경계를 넘는 위협 체계화 | 정량적 측정 방법론 부재 |

**다음 학습 방향 (Day 3 Preview):**

Day 3에서는 기술적 분석과 주제별 분석의 실제 결과를 다룬다. 5가지 리스크 유형의 세부 내용, 3차원 PoP의 구체적 특성, 공격 전-중-후 완화 전략의 실제 내용, 그리고 IT 보안-조직 보안-공급망 보안을 연결하는 통합 개념 모델이 어떻게 도출되었는지 확인할 수 있을 것이다.
...
| 유형 | 주요 내용 | 핵심 특성 |
|------|-----------|-----------|
| **물리적 위협** | 서버·라우터 등 ICT 장비에 대한 자연재해, 물리적 파괴, 테러 공격 | 많은 리스크 관리자가 간과하는 유형 |
| **시스템 고장** | 노후 방화벽, 보안 업데이트 지연 등으로 인한 시스템 또는 자원의 중단 | 예측 가능성이 높으나 파급 효과는 의도적 공격과 동등할 수 있음 |
| **간접 공격** | 바이러스·웜·트로이목마, 위조 제품·소프트웨어·하드웨어, 악성코드, 스푸핑, 피싱 등 미끼를 통한 시스템 침투 | 직원이 미끼를 수용함으로써 공격자가 시스템에 접근하는 구조 |
| **직접 공격** | 해킹, 서비스 거부 공격(DoS), 비밀번호 스니핑, 산업 스파이, 지식재산 침해 등 금전적 이익을 위한 직접 침입 | 의도적이고 표적화된 공격 |
| **내부자 위협** | 직원의 부주의(비밀번호 관리 소홀, 민감 정보 노출)부터 의도적 데이터 오용, 사보타주까지 | 가장 예측 불가하고 탐지하기 어려운 위협 유형 |
...
[3차 전파: 사회 전반 영향]
  - 최종 소비자 직접 피해
    (예: 자동차 제동 시스템 악성코드 감염)
  - 공중 보건 관련 공급망 공격 시 사회적 파장
    (식품, 제약 공급망)
  - 이익률, 시가총액, 브랜드 이미지 손상
```
...
**조직 간 협업(Inter-organizational Collaboration):**
...
**정부 관여(Governmental Involvement):**
...
**핵심 발견:** 기존 문헌의 대부분이 공격 전 단계 대응에만 집중하며, 공격 중·공격 후 단계 완화 방안은 현저히 부족하다. 이는 향후 연구의 우선 과제다.
...
기업들은 사이버 보안을 기술적 문제로만 인식하는 경향이 있지만, 기술적 보안이 강화될수록 공격자는 더 취약한 인적 경로로 이동한다. Kunnathur(2015)는 미래의 의도적 공격이 기술적 영역보다 인적 PoP를 더 집중적으로 활용할 것이라 예측했다. 조직 경계를 가로지르는 공급망 직원들의 상호작용은 이 취약성을 더욱 증폭시킨다.
...
## Tags
`#보안컨설팅` `#SecurityConsulting` `#SupplyChainSecurity` `#CyberRisk` `#CSCRM` `#ThirdPartyRisk` `#PaperReview` `#SKShieldusRookies`